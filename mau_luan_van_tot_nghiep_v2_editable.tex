...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    \item \textbf{Chưa đủ kiểm chứng ngoài tập dữ liệu (Lack of Out-of-Distribution Testing):} Mặc dù các số liệu thực nghiệm nội bộ rất khả quan (Accuracy 96\%, F1-Score 93\%), hệ thống vẫn chưa được kiểm định hệ thống trên một tập dữ liệu ngoại lai hoàn toàn độc lập (External Test Set). Nếu không có bài test này, không thể khẳng định chắc chắn mô hình có năng lực khái quát hóa tốt khi đối mặt với các dạng tin giả hoàn toàn mới trong tương lai hay không.
    \item \textbf{Rủi ro "Học vẹt nguồn dữ liệu" (Source Memorization):} Đây là cạm bẫy lớn nhất của bài toán NLP. Do chỉ sử dụng chia tách ngẫu nhiên phân tầng (Stratified Split) thay vì chia tách theo nguồn (Source Split), mô hình có rủi ro rất cao bị "học vẹt" cấu trúc hoặc văn phong đặc thù của một tòa soạn nhất định. Mặc dù đã dùng thuật toán \texttt{drop\_duplicates()} để xóa bỏ các bài báo trùng lặp hoàn toàn nhằm chống rò rỉ dữ liệu (Data Leakage), nhưng việc rò rỉ văn phong (Stylistic Leakage) giữa tập Train và tập Test vẫn khiến hiệu năng đánh giá nội bộ có phần được "tô hồng" so với thực tế.
