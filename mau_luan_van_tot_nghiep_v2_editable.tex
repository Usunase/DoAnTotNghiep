...
Trong bối cảnh bùng nổ thông tin tại Việt Nam, bài toán phát hiện tin giả tiếng Việt đã thu hút sự chú ý mạnh mẽ từ cộng đồng xử lý ngôn ngữ tự nhiên (NLP). 
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
...
\begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{0.5cm}|>{\raggedright\arraybackslash}p{2.5cm}|>{\raggedright\arraybackslash}p{3.5cm}|X|X|}
\hline
...
\hline
1 & VLSP ReINTEL (2020) & PhoBERT + Ensemble Learning & Khẳng định sức mạnh của PhoBERT trong việc trích xuất vector ngữ nghĩa tiếng Việt. & Tập trung vào tối ưu hóa "hộp đen" (Black-box), hoàn toàn thiếu vắng cơ chế diễn giải cho dự đoán. \\
\hline
2 & dEFEND (2019) & Hierarchical Attention Networks & Tiên phong đưa XAI vào NLP thông qua việc trực quan hóa sự chú ý (Attention) trên bình luận. & Phương pháp giải thích phụ thuộc vào dữ liệu tương tác mạng xã hội, không phù hợp cho phát hiện sớm (Early Detection). \\
\hline
...
\hline
\textbf{5} & \textbf{ShieldAI (Tiểu luận)} & \textbf{PhoBERT Fine-tuned + Heuristic XAI} & \textbf{Phân tách kiến trúc: sử dụng PhoBERT để phân loại chính xác, kết hợp một lớp XAI Heuristic giải thích bề mặt với chi phí tính toán O(1).} & \textbf{Phương pháp XAI chỉ mang tính suy diễn bề mặt (Post-hoc), chưa mổ xẻ được cơ chế suy luận nội tại của mạng Transformer.} \\
\hline
\end{tabularx}
\end{table}

Phân tích trên cho thấy hai rào cản lớn nhất của hệ thống học sâu hiện tại là tính "hộp đen" và độ phức tạp tính toán khi triển khai thực tế. Khoảng trống nghiên cứu này đòi hỏi một phương pháp cân bằng giữa độ chính xác của Transformer và tính minh bạch, nhưng không được phép làm tăng chi phí tính toán. Hệ thống \textbf{ShieldAI} trong tiểu luận này được thiết kế để giải quyết bài toán trên thông qua tư duy \textit{Phân tách Ngữ nghĩa - Kinh nghiệm (Decoupled Semantic-Heuristic)}: nhường hoàn toàn tác vụ phân loại cho kiến trúc PhoBERT Sequence Classification nguyên bản (tối đa tốc độ), đồng thời vận hành một module giải thích Heuristic ngoại vi (Post-hoc Rule-based) để trực quan hóa các thủ thuật thao túng văn bản, bù đắp trực tiếp vào khiếm khuyết của các phương pháp "hộp đen" mà không can thiệp vào đồ thị tính toán cốt lõi.
