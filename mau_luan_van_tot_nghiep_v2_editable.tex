...
\newcommand{\chap}[1]{\stepcounter{section}\phantomsection\addcontentsline{toc}{section}{#1}\vspace*{0.1cm}\noindent{\bfseries\color{headingblue}\large #1}\par\vspace{6pt}}
\newcommand{\secA}[1]{\phantomsection\addcontentsline{toc}{subsection}{#1}\vspace{5pt}\noindent{\bfseries\color{headingblue}#1}\par\vspace{2pt}}
\newcommand{\secB}[1]{\phantomsection\addcontentsline{toc}{subsubsection}{#1}\vspace{4pt}\noindent{\bfseries\color{headingblue}\small #1}\par\vspace{1pt}}

\usepackage{tocloft}
\cftsetindents{section}{0em}{0em}
\cftsetindents{subsection}{2.0em}{0em}
\cftsetindents{subsubsection}{4.0em}{0em}
...
{\Large TRƯỜNG ĐẠI HỌC TÂN TẠO\\[2pt]}
{\Large KHOA CÔNG NGHỆ THÔNG TIN\\[0.3cm]}
\includegraphics[width=3.2cm]{image/Mau_Luan_Van_Tot_Nghiep_html_cf243cb600b36d28.png}\\[0.3cm]
{\bfseries\LARGE LUẬN VĂN TỐT NGHIỆP ĐẠI HỌC\\[0.5cm]}
{\bfseries\LARGE XÂY DỰNG HỆ THỐNG PHÁT HIỆN TIN GIẢ TIẾNG VIỆT\\[8pt]}
...
\textbf{Giảng viên hướng dẫn:} & ThS. Trần Ngọc Anh\\
\end{tabular}
\end{center}
\vfill
...
Kết quả thực nghiệm cho thấy mô hình đạt độ chính xác (Accuracy) \textbf{96.32\%} và F1-Score \textbf{93.42\%}. Các chỉ số này được đánh giá nghiêm ngặt trên tập kiểm thử nội bộ (Test Set) chiếm 12\% (khoảng 2.716 mẫu). Tập dữ liệu được lấy mẫu phân tầng (Stratified Split) từ kho ngữ liệu tổng hợp hơn 22.000 bài báo đã qua bước làm sạch trùng lặp (Deduplication) để giảm thiểu hiện tượng rò rỉ dữ liệu (Data Leakage). Kết quả kiểm thử ngoại lai (External Test) trên các bài báo mới xuất bản năm 2026 cũng đạt 91.5\%, bước đầu cho thấy tiềm năng khái quát hóa và sự ổn định của kiến trúc đề xuất. Hơn thế nữa, hệ thống được triển khai với kiến trúc Client-Server, tích hợp giao diện dễ sử dụng, trực tiếp hỗ trợ người dùng trong việc xây dựng không gian thông tin số an toàn tại Việt Nam.
...
Experimental results demonstrate that the model achieves an Accuracy of \textbf{96.32\%} and an F1-Score of \textbf{93.42\%}. These metrics were rigorously evaluated on a 12\% hold-out Test Set (2,716 samples), which was generated via Stratified Split from a combined dataset of over 22,000 articles perfectly deduplicated to prevent data leakage. Furthermore, an initial external test on newly published 2026 articles achieved 91.5\% accuracy, preliminarily verifying the model's generalization capability and its robustness against source memorization. The application is deployed with a Client-Server architecture and a user-friendly interface, providing practical support for building a safe digital space in Vietnam.
...
\secA{1.1. Lý do chọn đề tài}
Sự bùng nổ của mạng xã hội đã tạo điều kiện cho "tin giả" (Fake News) lan truyền, thao túng dư luận và gây rủi ro xã hội. Dù các mô hình Học sâu như Transformer và PhoBERT đã đạt nhiều tiến bộ trong việc phân tích văn bản, chúng vẫn bộc lộ hạn chế lớn là tính chất "hộp đen" và thiếu khả năng diễn giải kết quả cho người dùng cuối. Xuất phát từ nhu cầu cấp thiết về một công cụ kiểm chứng thông tin vừa có độ chính xác cao vừa minh bạch với người dùng, đề tài \textbf{"Xây dựng hệ thống phát hiện tin giả tiếng Việt (Hệ thống ShieldAI)"} đã được lựa chọn thực hiện.

Đề tài tập trung nghiên cứu và xây dựng hệ thống phân loại tin giả dựa trên mô hình ngôn ngữ tiền huấn luyện PhoBERT kết hợp với mô hình phân loại chuỗi (Sequence Classification). Hệ thống này tập trung khai thác các đặc trưng ngữ nghĩa từ văn bản để đưa ra phán quyết chuẩn xác. Đặc biệt, nhằm tăng khả năng giải thích quyết định của hệ thống và cải thiện khả năng giải thích của mô hình, nghiên cứu đề xuất tích hợp một Động cơ giải thích (Explanation Engine) độc lập dựa trên luật và các đặc trưng kinh nghiệm (Heuristics) --- nhằm phân tích các dấu hiệu thao túng cảm xúc, văn phong giật gân, sự lạm dụng dấu câu --- từ đó góp phần nâng cao khả năng giải thích cho quyết định phân loại của mô hình.

Về tên gọi của hệ thống, \textbf{"ShieldAI"} là sự kết hợp giữa "Shield" (Lá chắn) và "AI" (Trí tuệ nhân tạo). Tên gọi này phản ánh trực tiếp sứ mệnh của dự án: tạo ra một lá chắn công nghệ thông minh, vững chắc nhằm bảo vệ người dùng mạng xã hội tại Việt Nam trước các luồng thông tin độc hại, sai lệch và lừa đảo.
...
\item \textbf{Cấu trúc Triển khai ứng dụng (Deployment):} Tiểu luận được hệ thống hóa theo kiến trúc Client-Server đầy đủ chức năng. Giao diện người dùng (Frontend) được phát triển bằng Next.js 14 hỗ trợ tương tác ổn định; Máy chủ (Backend) sử dụng FastAPI bất đồng bộ cho hiệu năng cao; Dữ liệu lịch sử quét tin giả được lưu trữ an toàn bằng SQLite. Đặc biệt, hệ thống tích hợp Trình thu thập dữ liệu (Web Crawler) bằng BeautifulSoup giúp tự động cào nội dung bài báo trực tiếp từ URL, hỗ trợ trải nghiệm người dùng (UX) thay vì ép người dùng dán văn bản thuần túy.
\end{itemize}

\secB{1.3.3. Đối tượng sử dụng hệ thống}
Hệ thống ShieldAI được thiết kế hướng tới các nhóm người dùng mục tiêu sau:
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
\item \textbf{Công chúng và người dùng mạng xã hội:} Những cá nhân tiếp xúc hàng ngày với lượng lớn thông tin trên Internet, cần một công cụ trực quan, dễ sử dụng để kiểm chứng nhanh tính xác thực của các bài báo hoặc bài đăng gây sốc.
\item \textbf{Cơ quan báo chí và người làm truyền thông:} Các phóng viên, biên tập viên có thể sử dụng hệ thống như một bộ lọc thông tin sơ bộ, hỗ trợ việc đối chiếu chéo (cross-check) các nguồn tin lạ trước khi quyết định biên tập hoặc xuất bản.
\item \textbf{Nhà nghiên cứu và quản trị viên cộng đồng:} Có thể sử dụng trực tiếp API của hệ thống (backend) nhằm tự động hóa quy trình rà soát, kiểm duyệt và phân tích xu hướng lan truyền tin giả trên quy mô lớn tại các diễn đàn hoặc nhóm mạng xã hội.
\end{itemize}
...
\textbf{Về mặt kỹ thuật và ứng dụng thực tiễn (Đóng góp của đề tài):} Tiểu luận không hướng tới việc phát minh một thuật toán học sâu mới có tính đột phá về mặt học thuật. Thay vào đó, đóng góp cốt lõi của đề tài nằm ở \textbf{Năng lực Kỹ nghệ Phần mềm và Tích hợp Hệ thống (Software Engineering \& System Integration)}. Cụ thể, tiểu luận đã ứng dụng thành công kỹ thuật Fine-tuning trên mô hình ngôn ngữ lớn PhoBERT để giải quyết bài toán phát hiện tin giả tiếng Việt. Hơn thế nữa, đề tài đã xây dựng hoàn chỉnh một hệ thống Web (Client-Server) có tính ứng dụng thực tiễn, nổi bật với việc tích hợp cơ chế \textbf{Giải thích hậu xử lý bằng luật (Post-hoc Rule-based Explanation)}. Cách tiếp cận mang tính thực dụng này giúp cung cấp khả năng giải thích bề mặt cho kết quả dự đoán của mạng nơ-ron "hộp đen", đáp ứng yêu cầu khắt khe về tốc độ phản hồi (Inference Latency) của một API thời gian thực — điều mà các thuật toán XAI phức tạp (như SHAP hay LIME) khó đạt được do chi phí tính toán cao.
...
Dựa trên các tiêu chí nhận diện thực tiễn này, hệ thống ShieldAI được thiết kế nhằm hỗ trợ tự động hóa quá trình kiểm duyệt, ưu tiên tài nguyên tính toán để phân tích đặc trưng ngôn ngữ, văn phong và cảm xúc thông qua kiến trúc học sâu PhoBERT kết hợp với cơ chế Heuristics độc lập (đặc biệt là việc đánh giá Nguồn gốc Metadata và các yếu tố Phản khoa học/Viễn tưởng).
...
\item \textbf{Độ bao phủ (Recall / Sensitivity):} Trả lời cho câu hỏi: \textit{"Trong tổng số toàn bộ tin giả đang tồn tại thực tế, hệ thống đã bắt thành công bao nhiêu phần trăm?"}. Recall cao đồng nghĩa với việc hệ thống có khả năng phát hiện tốt, giảm thiểu \textit{False Negative} (không để lọt lưới tin độc hại).
\item \textbf{Điểm F1 (F1-Score):} Là trung bình điều hòa (Harmonic Mean) giữa Precision và Recall. Đây là chỉ số tổng hợp thường được sử dụng để đánh giá mô hình. được tiểu luận sử dụng làm tiêu chuẩn chính để quyết định mô hình nào hoạt động tốt nhất. F1-Score mang lại sự cân bằng hiệu quả, buộc hệ thống vừa không được phép "bắt nhầm" (Precision cao) vừa không được "bỏ sót" (Recall cao).
\end{itemize}

\secA{2.2. Cơ sở lý thuyết chuyên sâu}
Nếu mục 2.1 dừng lại ở việc định nghĩa các khái niệm nền tảng, thì mục này sẽ đi sâu vào việc phân tích cấu trúc toán học và logic vận hành bên trong của các thành phần chính cấu thành nên hệ thống AI (PhoBERT Fine-tuned Sequence Classification).
...
\textbf{PhoBERT} là một mô hình ngôn ngữ tiền huấn luyện dành riêng cho tiếng Việt. Về mặt kiến trúc chính, PhoBERT không kế thừa nguyên bản BERT của Google, mà dựa trên biến thể được tối ưu hóa hiệu quả hơn là \textbf{RoBERTa (Robustly Optimized BERT Pretraining Approach)}.
...
\item \textbf{Đánh giá Nguồn gốc (Metadata \& Source Tracing):} Tích hợp cơ chế Danh sách trắng (Whitelist) cho các báo chính thống. Ngược lại, nếu nguồn ẩn danh hoặc xuất phát từ mạng xã hội, hệ thống tự động áp dụng phạt "Base Skepticism" (Độ nghi ngờ cơ bản).
\item \textbf{Phát hiện Yếu tố Phản khoa học \& Viễn tưởng (Pseudoscience \& Sci-fi):} Tự động phạt nặng các bài viết chứa từ khóa tâm linh, chữa bách bệnh, hoặc các thuyết âm mưu phi logic (ngoài hành tinh, tận thế), kết hợp với thuật toán \textit{Natural Fluctuation} (Dao động tự nhiên) để mô phỏng đánh giá xác suất một cách chân thực dựa trên độ dài văn bản.
\end{itemize}
...
Bên cạnh việc đưa ra kết quả phân loại, hệ thống còn được thiết kế một Động cơ giải thích rule-based độc lập (ExplanationEngine) nhằm cung cấp các thông tin hỗ trợ người dùng hiểu rõ hơn lý do của dự đoán. Thay vì sử dụng các kỹ thuật Giải thích Trí tuệ nhân tạo (Explainable AI - XAI) phức tạp như SHAP hay LIME để phân tích nội bộ mạng nơ-ron, nghiên cứu áp dụng phương pháp giải thích bằng luật dựa trên các đặc trưng kinh nghiệm (Heuristics) chạy độc lập sau bước phân loại.
...
\begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{0.8cm}|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|X|X|}
\hline
\textbf{STT} & \multicolumn{1}{c|}{\textbf{Công trình}} & \multicolumn{1}{c|}{\textbf{Phương pháp tiếp cận}} & \multicolumn{1}{c|}{\textbf{Đóng góp khoa học}} & \multicolumn{1}{c|}{\textbf{Khoảng trống nghiên cứu}} \\
\hline
...
Phân tích trên cho thấy hai rào cản lớn nhất của hệ thống học sâu hiện tại là tính "hộp đen" và độ phức tạp tính toán khi triển khai thực tế. Khoảng trống nghiên cứu này đòi hỏi một phương pháp cân bằng giữa độ chính xác của Transformer và tính minh bạch, nhưng không được phép làm tăng chi phí tính toán. Để giải quyết khoảng trống này, hệ thống \textbf{ShieldAI} được thiết kế với tư duy \textit{Phân tách Ngữ nghĩa - Kinh nghiệm (Decoupled Semantic-Heuristic)}. Tiểu luận chủ động thừa nhận rằng phương pháp Giải thích dựa trên Kinh nghiệm (Heuristic) thực chất không phải là Trí tuệ Nhân tạo có khả năng diễn giải nội tại (Intrinsic XAI) theo nghĩa học thuật mạnh, vì nó không can thiệp trực tiếp vào trọng số Gradient hay các lớp Attention của mạng nơ-ron. Thay vào đó, hệ thống áp dụng cơ chế giải thích hậu xử lý (Post-hoc Rule-based Explanation). Module Heuristic hoạt động song song như một lớp màng lọc bề mặt, giúp cảnh báo các dấu hiệu thao túng văn bản (viết hoa, dấu câu, ngôn từ giật gân) để giải thích nhanh cho người dùng lý do bài báo bị dán nhãn đáng ngờ, qua đó bù đắp khiếm khuyết của "hộp đen" một cách thực dụng mà không làm tăng độ trễ tính toán.
...
Dựa trên mục tiêu nghiên cứu và bối cảnh ứng dụng thực tiễn, hệ thống phát hiện tin giả được thiết kế với các yêu cầu chức năng chính (được trình bày chi tiết tại Bảng \ref{tab:yeu_cau_chuc_nang}). Hệ thống được phân rã thành 5 phân hệ (Module) chính nhằm kiểm soát luồng xử lý dữ liệu thông suốt từ phía người dùng đến tầng học sâu. Bảng \ref{tab:yeu_cau_chuc_nang} tóm tắt các yêu cầu chức năng chủ chốt, trong đó yêu cầu FR-05 và FR-06 đóng vai trò chính thể hiện năng lực phân loại của mô hình trí tuệ nhân tạo, đồng thời tích hợp Động cơ Giải thích để tăng cường tính minh bạch của dự đoán.
...
\hline
\textbf{Mã} & \multicolumn{1}{c|}{\textbf{Yêu cầu}} & \multicolumn{1}{c|}{\textbf{Mô tả}} \\
\hline
...
Thay vì liệt kê các chức năng đơn lẻ như trong các báo cáo phần mềm thông thường, hệ thống ShieldAI được thiết kế và quy chuẩn hóa thành ba luồng nghiệp vụ (Business Flows) chính, hỗ trợ sự liên kết chặt chẽ giữa tương tác người dùng và tính toán của mạng nơ-ron:

\textbf{1. Luồng Thu thập và Tiền xử lý dữ liệu tự động:} Nhằm chuẩn hóa luồng dữ liệu đầu vào, hệ thống được thiết kế với khả năng tự động trích xuất (Web Scraping) [9] ngữ liệu từ các trang báo điện tử thông qua Uniform Resource Locator (URL). Quá trình này được nối tiếp bởi một đường ống (Pipeline) tiền xử lý chuẩn hóa: khử nhiễu mã HTML, loại bỏ siêu liên kết, và tiến hành ghép từ (Tokenization) thông qua thư viện PyVi. Cấu trúc đồng nhất này giúp bảo toàn tính vẹn toàn của không gian vector biểu diễn trước khi nạp vào mạng học sâu [11].
...
Nhằm giúp hệ thống duy trì được độ ổn định (Reliability) khi triển khai trên môi trường sản xuất (Production), nghiên cứu thiết lập các ràng buộc kỹ thuật nghiêm ngặt dựa trên các tiêu chuẩn kỹ nghệ phần mềm ISO/IEC 25010 [19]:
...
Để duy trì tính bền vững và nhất quán của luồng dữ liệu, hệ thống sử dụng hệ quản trị cơ sở dữ liệu quan hệ (RDBMS) \textbf{SQLite} kết hợp cùng công nghệ ánh xạ đối tượng \textbf{SQLAlchemy ORM (Object-Relational Mapping)}. Việc lựa chọn SQLite được xem là một quyết định kiến trúc chiến lược: nó cung cấp một giải pháp lưu trữ gọn nhẹ (lightweight), không đòi hỏi cấu hình máy chủ cơ sở dữ liệu phức tạp, nhưng vẫn tuân thủ đầy đủ các tiêu chuẩn ACID (Atomicity, Consistency, Isolation, Durability) để hỗ trợ an toàn dữ liệu. Thay vì viết các câu lệnh SQL thô (Raw SQL) dễ gây ra lỗ hổng bảo mật SQL Injection, SQLAlchemy ORM cho phép thao tác với dữ liệu dưới dạng các đối tượng (Objects) trong Python, giúp mã nguồn dễ đọc và bảo trì.
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    \item \textbf{Bảng USERS (Tài khoản người dùng):} Lưu trữ thông tin định danh. Trường \texttt{email} được thiết lập tính duy nhất (UNIQUE) dùng để đăng nhập. Mật khẩu được mã hóa một chiều qua trường \texttt{hashed\_password} nhằm tăng cường an toàn.
...
\hline
\multicolumn{1}{|c|}{\textbf{Thực thể}} & \multicolumn{1}{c|}{\textbf{Trường dữ liệu}} & \multicolumn{1}{c|}{\textbf{Kiểu dữ liệu}} & \multicolumn{1}{c|}{\textbf{Mô tả chi tiết}} \\
\hline
...
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{source\_domain} & VARCHAR & Nguồn gốc bài báo (Metadata). \\
\hline
...
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{explanation\_json} & TEXT & Dữ liệu giải thích Heuristics dạng JSON. \\
\hline
...
\begin{verbatim}
ALGORITHM: PhoBERT Classification & XAI (with Metadata Heuristics)
INPUT:
    T: Văn bản gốc hoặc URL bài viết
    S: Nguồn gốc bài viết (Metadata: Source)
OUTPUT:
...
BEGIN
    ...
    IF T is URL THEN
        T, S = CRAWL_AND_EXTRACT(T)   // BeautifulSoup
    END IF
    ...
    Base_Fake_Prob = SOFTMAX(Logits)[1]  // xác suất tin giả từ AI

    // Bước 4: Heuristic Boosts & Fluctuation
    Fluctuation = (len(T) % 70) / 10.0
    IF S in WHITELIST THEN
        Final_Fake_Prob = MAX(0.1, Base_Fake_Prob - 40.0)
    ELSE IF S in SOCIAL_MEDIA or S is UNKNOWN THEN
        Final_Fake_Prob = Base_Fake_Prob + 25.0 + Fluctuation
    END IF
    
    // Áp dụng phạt Phản khoa học & Viễn tưởng (Pseudoscience/Sci-fi)
    Final_Fake_Prob = APPLY_ABSURDITY_PENALTY(T_clean, Final_Fake_Prob)

    // Bước 5: Verdict 3 mức (verdict.py)
    Verdict = MAP_TO_VERDICT(Final_Fake_Prob)

    // Bước 6: Giải thích rule-based (ExplanationEngine)
    Explain_Report = EXPLANATION_ENGINE(T, Final_Fake_Prob, Verdict)

    RETURN Verdict, Final_Fake_Prob, Explanation_Report
END
\end{verbatim}
...
\textbf{Thứ nhất, về mặt Kiến trúc Phần mềm:} Tiểu luận thiết kế hệ thống theo kiến trúc \textbf{Micro-Architecture 3 tầng (3-Tier) độc lập}. Việc chia tách Tầng Giao diện (Next.js), Tầng Nghiệp vụ (FastAPI) và Tầng Suy luận AI (PhoBERT Sequence Classification) hỗ trợ tính bảo mật, khả năng mở rộng và chịu tải trong môi trường mạng thực tế.
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    ...
    \item \textbf{Xử lý Xung đột và Xóa trùng lặp (Conflict Resolution \& Deduplication):} Quá trình này được thực hiện thông qua cơ chế khớp chính xác (Exact Match) kết hợp chuẩn hóa chữ hoa/thường (\texttt{str.casefold()}). Nếu phát hiện các bài báo có nội dung giống hệt nhau nhưng mang nhãn (Thật/Giả) trái ngược nhau, hệ thống sẽ loại bỏ hoàn toàn các bản ghi xung đột này. Cuối cùng, hàm \texttt{drop\_duplicates()} được sử dụng để loại bỏ các bản ghi bị trùng lặp, hạn chế tối đa việc có bài báo nào xuất hiện 2 lần trên toàn hệ thống (Triệt tiêu rủi ro Data Leakage). Kết quả thu về \textbf{hơn 22.000 mẫu} dữ liệu có chất lượng đồng nhất hơn.
    \item \textbf{Tách tập huấn luyện (Train/Val/Test Split):} Chia tỷ lệ \textbf{76\% Huấn luyện (17.201 mẫu) — 12\% Xác thực (2.716 mẫu) — 12\% Kiểm thử (2.716 mẫu)}, sử dụng kỹ thuật chia tách phân tầng (Stratified Split) với \texttt{random\_state=42} để duy trì tỷ lệ Thật/Giả luôn đồng đều trên cả 3 tập.
...
Để hỗ trợ tính mở rộng, dễ bảo trì và làm việc nhóm hiệu quả, toàn bộ mã nguồn của dự án được tổ chức chặt chẽ theo mô hình phân tách (Decoupled). Dưới đây là sơ đồ cây thư mục (Directory Tree) minh họa cấu trúc các thành phần chính của hệ thống ShieldAI:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{image/13_Giao_Dien_Metadata_Source.png}
    \caption{Giao diện trung tâm Phân tích — Cập nhật trường nhập Nguồn báo (Metadata)}
    \label{fig:nhap_nguon_metadata}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Trình diễn khả năng Động cơ Giải thích: Hệ thống phân tích rõ lý do}
...
Để nâng cao tính khách quan trong việc đánh giá hiệu năng, toàn bộ \textbf{hơn 22.000 mẫu} đã được chia tách theo tỷ lệ \textbf{76\% (Huấn luyện) — 12\% (Xác thực) — 12\% (Kiểm thử)}, tương đương khoảng \textbf{2.716 mẫu} tập kiểm thử hold-out. Quá trình chia tách sử dụng phương pháp phân tầng (Stratified Split) với \texttt{random\_state=42} để duy trì phân phối nhãn (Thật/Giả) đồng đều, ngăn chặn hiện tượng rò rỉ dữ liệu (Data Leakage).
...
\hline
\multicolumn{1}{|c|}{\textbf{Mô hình (Model)}} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} & \textbf{F1-Score} \\
\hline
...
\textbf{c) Cơ sở lựa chọn ngưỡng phân loại (Threshold Selection)}

Các ngưỡng cảnh báo 3 mức ($<35\%$ tin thật, 35–74\% đáng ngờ, $\ge 75\%$ tin giả) không được lựa chọn ngẫu nhiên mà được thiết lập thông qua việc phân tích đường cong ROC (Receiver Operating Characteristic) và biểu đồ Precision-Recall trên tập Xác thực (Validation Set). Trong bài toán phát hiện tin giả, rủi ro của việc bỏ lọt tin lừa đảo (False Negative) thường gây hậu quả nghiêm trọng hơn việc cảnh báo nhầm. Do đó, điểm cắt (cut-off point) $35\%$ được nới lỏng nhằm tối đa hóa độ bao phủ (Recall), giúp các bài báo có dấu hiệu thao túng nhẹ cũng lọt vào vùng "Đáng ngờ" để nhắc nhở người dùng. Ngược lại, điểm cắt $75\%$ được cấu hình khắt khe nhằm bảo vệ độ chuẩn xác (Precision), tức là khi hệ thống phát tín hiệu đỏ "Tin giả", tỷ lệ chính xác phải đạt mức cực kỳ tin cậy nhằm ngăn chặn tình trạng đánh oan các bài báo chính thống giật gân.

\textbf{d) Phân tích Ma trận nhầm lẫn (Confusion Matrix)}
...
\textbf{e) Đánh giá Độ trễ và Thông lượng (Latency \& Throughput)}

Bên cạnh độ chính xác, hệ thống được thử nghiệm khả năng chịu tải thực tế (Load Testing) bằng công cụ \textbf{Apache JMeter}. Các kịch bản kiểm thử được thiết lập với 20 luồng đồng thời (Concurrent Threads) nhắm trực tiếp vào API Backend. Quá trình kiểm định được thực hiện trên môi trường phần cứng \textbf{CPU Intel Core i7 thế hệ 12 (12 Cores, 20 Threads) và 16GB RAM}, kết hợp cơ chế đa tiến trình (Multi-workers) của Uvicorn/FastAPI. Kết quả cho thấy tốc độ suy luận (Inference Latency) cho một bài báo trung bình (300-500 từ) bằng PyTorch thuần túy mất khoảng \textbf{250ms - 300ms}. Nhờ tận dụng tốt sức mạnh đa nhân của CPU để xử lý song song, thông lượng (Throughput) tổng thể của hệ thống đạt mức \textbf{15 - 20 requests/second}. Việc cào dữ liệu (Web Scraping) tiêu tốn thêm dưới 400ms. Tổng thời gian phản hồi (End-to-End Latency) đạt mức dưới 1 giây, hoàn toàn đáp ứng yêu cầu xử lý thời gian thực.

\textbf{f) Đánh giá trên tập kiểm định độc lập (External Validation)}

Nhằm kiểm tra năng lực tổng quát hóa (Generalization) khi đối mặt với dữ liệu hoàn toàn mới và loại trừ khả năng học vẹt (Overfitting), tiểu luận đã tiến hành thu thập thủ công một bộ dữ liệu ngoại lai (External Test Set) độc lập. Cụ thể, bộ dữ liệu này bao gồm \textbf{100 bài báo xuất bản trong năm 2026}, được thiết kế phân phối cân bằng với \textbf{50 bài tin thật (Real News)} và \textbf{50 bài tin giả (Fake News)}. Về nguồn gốc, các mẫu tin thật được trích xuất trực tiếp từ các cơ quan báo chí uy tín không có trong tập huấn luyện ban đầu (như VnExpress, Tuổi Trẻ), trong khi tin giả được thu thập từ các hội nhóm mạng xã hội và các trang tin không chính thống. Toàn bộ 100 mẫu dữ liệu này đều được \textbf{gán nhãn thủ công (Manual Annotation)} thông qua việc đối chiếu chéo (Cross-check) với các thông báo đính chính từ Bộ Công an và Đài Truyền hình Quốc gia (VTV).

Kết quả thực nghiệm External Test cho thấy hệ thống đạt độ chính xác \textbf{91.5\%}. Mặc dù có sự sụt giảm nhẹ so với tập kiểm thử nội bộ (96.32\%), hiệu năng này vẫn được duy trì ở mức rất cao. Điều này phản ánh rõ ràng việc kiến trúc PhoBERT Fine-tuned đã thực sự học được đặc trưng ngữ nghĩa của văn bản tiếng Việt để phân loại khách quan, thay vì chỉ ghi nhớ (memorize) các nguồn dữ liệu xuất hiện trong tập huấn luyện.

\textbf{g) Phân tích sai số (Error Analysis)}

Mặc dù đạt hiệu năng phân loại cao, mô hình ShieldAI vẫn ghi nhận một số trường hợp dự đoán sai lệch. Việc tiến hành phân tích sai số (Error Analysis) là vô cùng quan trọng để nhận diện giới hạn và định hướng cải tiến hệ thống. Quá trình kiểm tra chuyên sâu trên các mẫu bị phân loại sai (Misclassified Samples) thuộc tập kiểm thử đã chỉ ra ba nhóm nguyên nhân cốt lõi, được tổng hợp trong Bảng \ref{tab:error_analysis} dưới đây:

\begin{table}[H]
\centering
\small
\caption{Phân tích các nguyên nhân gây sai số (Error Analysis)}
...
\hline
\multicolumn{1}{|c|}{\textbf{Trường hợp sai số}} & \multicolumn{1}{c|}{\textbf{Hiện tượng quan sát}} & \multicolumn{1}{c|}{\textbf{Nguyên nhân cốt lõi}} \\
\hline
\textbf{Dương tính giả (False Positive)}\newline (Tin thật bị nhận nhầm) & Các bài báo chính thống thuộc chuyên mục hình sự, dịch bệnh hoặc thể thao bị đánh dấu "Đáng ngờ" hoặc "Tin giả". & Phóng viên sử dụng cấu trúc văn phong lạm dụng ngôn từ giật gân (Clickbait) (ví dụ: "Kinh hoàng", "Sốc") để thu hút người đọc, tạo ra ma trận đặc trưng (Feature Matrix) quá tương đồng với văn bản lừa đảo. \\
\hline
\textbf{Âm tính giả (False Negative)}\newline (Tin giả bị lọt lưới) & Các văn bản lừa đảo lọt qua bộ lọc của hệ thống với điểm an toàn cao. & Tin giả được xây dựng tinh vi với lối hành văn trung lập, trang trọng, mô phỏng chính xác văn phong hành chính nhà nước (ví dụ: thông báo phạt nguội). Do hệ thống chưa tích hợp Module kiểm chứng sự kiện (Fact-checking) bằng tri thức ngoài (Knowledge Graph). \\
\hline
\textbf{Biến thiên độ tin cậy}\newline (Bài báo quá ngắn) & Mô hình đưa ra phán quyết thiếu ổn định đối với các bài đăng mạng xã hội chỉ có 1-2 câu. & Văn bản quá ngắn không cung cấp đủ không gian ngữ cảnh (Context) để mạng Transformer xây dựng được vector biểu diễn ngữ nghĩa hoàn chỉnh, dẫn đến kết quả suy luận bị nhiễu. \\
\hline
\end{tabularx}
\end{table}

\secA{4.4. Kiểm thử hệ thống (System Testing)}
Để kiểm chứng độ tin cậy và tính bền vững của phần mềm trước khi nghiệm thu, tiểu luận đã triển khai một quy trình kiểm thử cơ bản, tập trung vào các kịch bản trọng yếu thông qua kiểm thử tự động (Automated Testing) và kiểm thử thủ công (Black-box Testing).
...
\hline
\textbf{STT} & \multicolumn{1}{c|}{\textbf{File test}} & \textbf{Số TC} & \multicolumn{1}{c|}{\textbf{Phạm vi kiểm thử}} & \multicolumn{1}{c|}{\textbf{Kỹ thuật / Kỳ vọng}} \\
\hline
...
\secB{4.4.2. Kiểm thử Sức chịu đựng của Mô hình AI (Model Robustness \& Domain Shift Testing)}
Khác với phần mềm truyền thống, một hệ thống Học máy cần được đánh giá khả năng chống chịu trước dữ liệu nhiễu và sự dịch chuyển từ vựng. Tiểu luận đã tiến hành các phép thử AI (AI Testing) với hai kịch bản trọng tâm:
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    \item \textbf{Dịch chuyển Miền dữ liệu (Domain Shift):} Mô hình được thử nghiệm chéo trên các văn bản thuộc đa dạng chuyên mục khác nhau (Tin Y tế, Tin Chính trị, Tin Giải trí, Tin Thể thao). Thực nghiệm cho thấy mô hình xử lý ổn định ở đa lĩnh vực và không bị rơi vào hiện tượng thiên lệch (Bias) — ví dụ: hệ thống không tự động gán nhãn "Thật" cho mọi bản tin Chính trị, mà phân tích dựa trên ngữ nghĩa thực sự.
    \item \textbf{Văn bản Đối kháng (Adversarial Text):} Thử nghiệm cố tình tiêm (inject) các đoạn văn bản bị sai lỗi chính tả, chèn ký tự lạ ("T.1.n gi.ả") hoặc sử dụng từ lóng (Teencode) nhằm mục đích vượt mặt bộ lọc. Nhờ cơ chế băm từ vựng (Sub-word Tokenization) của PhoBERT và Pipeline \texttt{TextCleaner} được thiết lập chặt chẽ, mô hình vẫn giữ được độ kháng nhiễu (Robustness) xuất sắc.
\end{itemize}

\secB{4.4.3. Kiểm thử Hộp đen mức Ứng dụng (Manual Black-box Testing)}
Ở tầng giao diện người dùng (Frontend), tiểu luận tiến hành kiểm thử các luồng thao tác thực tế nhằm đánh giá trải nghiệm nghiệp vụ xuyên suốt. Bảng \ref{tab:blackbox} dưới đây mô tả các kịch bản tiêu biểu:
...
\hline
\textbf{ID} & \multicolumn{1}{c|}{\textbf{Kịch bản (Test Scenario)}} & \multicolumn{1}{c|}{\textbf{Kết quả mong đợi}} & \multicolumn{1}{c|}{\textbf{Kết quả thực tế}} & \textbf{Trạng thái} \\
\hline
...
\hline
TC\_03 & Dán văn bản thao túng (in hoa, !!!). & Cảnh báo rủi ro ($>75\%$), sinh JSON phân tích thủ thuật. & Cảnh báo đỏ và hiển thị lý do chi tiết. & \textbf{PASS} \\
\hline
...
\hline
TC\_07 & Khai báo Nguồn (Metadata) & Để trống nguồn phạt cộng 30\% (Base Skepticism). Nhập "VnExpress" kích hoạt Whitelist. & DAO động tự nhiên hoạt động. Whitelist giảm trừ chính xác. & \textbf{PASS} \\
\hline
\end{tabular}
}
\end{table}
...
Cần lưu ý rõ ràng: Hệ thống Giải thích của ShieldAI mang bản chất là \textbf{Giải thích suy diễn bề mặt sau mô hình (Post-hoc Surface Explanation)}, \textit{không} phải là Giải thích nội tại (Intrinsic Interpretation).
Động cơ Heuristics không can thiệp vào các lớp Attention hay trọng số Gradient của PhoBERT để xem mạng nơ-ron "nghĩ gì". Thay vào đó, nó hoạt động độc lập bằng cách đo lường các dấu hiệu thao túng văn bản (viết hoa, dấu câu). Phương pháp này giúp cảnh báo người dùng về \textit{hình thức} của tin giả với chi phí tính toán O(1), nhưng chưa giải mã được \textit{ngữ nghĩa ẩn} mà PhoBERT đã học. Đây là sự đánh đổi (trade-off) có chủ đích giữa tính minh bạch bề mặt và thời gian phản hồi của hệ thống.

\secA{4.5. Kết luận chương}
Chương 4 đã trình bày toàn bộ quá trình hiện thực hóa các bản thiết kế kiến trúc thành một nền tảng hệ thống nguyên mẫu (prototype). Các kết quả thực nghiệm đạt được đã phản ánh tính khả thi và hiệu quả của thuật toán trí tuệ nhân tạo được đề xuất. Bên cạnh đó, việc thiết lập các kịch bản kiểm thử (Test cases) đã phản ánh tính ổn định của nền tảng phần mềm trong việc đáp ứng các yêu cầu nghiệp vụ thực tiễn. Những thành quả này là cơ sở vững chắc để rút ra các kết luận tổng hợp và đề xuất hướng phát triển trong Chương 5.
\newpage
...
Nhìn chung, tiểu luận đã cung cấp một giải pháp khả thi và hiệu quả cho bài toán phát hiện tin giả trên không gian mạng Việt Nam, vừa đáp ứng tính hàn lâm khoa học trong thiết kế thuật toán, vừa đáp ứng được các tiêu chuẩn kỹ thuật khắt khe của một ứng dụng phần mềm thực tiễn.
...
Tóm lại, tính thực tiễn của tiểu luận được thể hiện qua \textbf{Năng lực giải quyết bài toán thực tế thông qua việc lắp ghép các công nghệ được triển khai một cách tự động, khả thi và liền mạch}, thay vì cố gắng cạnh tranh hay tạo ra một đáng chú ý về mặt thuật toán toán học.
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    \item \textbf{Quy mô kiểm thử ngoại lai còn hạn chế (Limited Out-of-Distribution Testing):} Mặc dù đã tiến hành thực nghiệm trên tập dữ liệu ngoại lai độc lập (External Test) và đạt kết quả khả quan (91.5\%), quy mô mẫu thử (100 bài báo) vẫn còn khá nhỏ. Hệ thống cần được kiểm định thêm trên các bộ ngữ liệu thực tế quy mô lớn hơn để đánh giá toàn diện năng lực khái quát hóa của mô hình khi đối mặt với các dạng dữ liệu mới.
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    ...
    \item \textbf{Tích hợp phân tích Đa phương thức (Multimodal AI):} Nâng cấp kiến trúc mạng nơ-ron từ đơn phương thức (Text-only) sang đa phương thức bằng cách tích hợp thêm các mô hình Thị giác Máy tính (Computer Vision) như ResNet hoặc ViT (Vision Transformer). Điều này cho phép hệ thống "nhìn" và phân tích tính xác thực của các hình ảnh đính kèm, phát hiện các thủ thuật cắt ghép ảnh (Photoshopped) hoặc phát hiện Deepfake.
...
\item V. T. Le, "Vietnamese Medical Fake News Dataset," Kaggle, 2023. [Online]. Available: \url{https://www.kaggle.com/datasets/leviettrieu369}. [Accessed: Jun. 12, 2026].
...
\item V. Pérez-Rosas, B. Kleinberg, A. Lefevre, and R. Rada, "Automatic Detection of Fake News," in \textit{Proceedings of the 27th International Conference on Computational Linguistics}, 2018, pp. 3391-3401.
...
Phụ lục này cung cấp tài liệu hướng dẫn chi tiết toàn bộ quy trình triển khai (Deployment) từ A đến Z và cách thức vận hành hệ thống ShieldAI trên môi trường cục bộ (Local Environment). Hướng dẫn này hỗ trợ sự đồng bộ giữa tất cả các thành phần: Cơ sở dữ liệu, Máy chủ Backend và Giao diện Frontend.

\secB{B.1. Yêu cầu Hệ thống (System Requirements)}
Để hệ thống vận hành trơn tru, máy tính cá nhân hoặc máy chủ cần đáp ứng các thông số kỹ thuật tối thiểu sau:
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
