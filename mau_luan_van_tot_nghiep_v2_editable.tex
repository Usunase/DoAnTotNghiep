...
\usepackage[left=3.1cm,right=2.4cm,top=1.75cm,bottom=2.0cm,headheight=14pt,headsep=0.35cm,footskip=0.9cm]{geometry}
\usepackage{fontspec}
\setmainfont{Tinos}
\usepackage{graphicx}
\usepackage{float}
...
\vspace{0.5cm}
Sự bùng nổ của mạng xã hội và truyền thông số tại Việt Nam đã mang lại nhiều lợi ích to lớn, nhưng đồng thời cũng kéo theo hệ lụy nghiêm trọng từ việc lan truyền tin giả (Fake News). Tin giả không chỉ gây hoang mang dư luận mà còn ảnh hưởng tiêu cực đến an ninh, chính trị và nền kinh tế. Nhằm giải quyết vấn đề cấp thiết này, tiểu luận trình bày quá trình nghiên cứu và phát triển hệ thống ShieldAI – một ứng dụng Web thông minh có khả năng tự động phát hiện và phân loại độ tin cậy của văn bản tiếng Việt.

Cốt lõi của hệ thống là mô hình học sâu \textbf{PhoBERT} được tinh chỉnh toàn diện theo kiến trúc Phân loại Chuỗi (Fine-tuned Sequence Classification) trên tập dữ liệu tiếng Việt đa dạng. Điểm nổi bật của ShieldAI là sự tích hợp cơ chế giải thích dựa trên đặc trưng kinh nghiệm (Heuristics-based Explanation). Cơ chế này giúp khắc phục nhược điểm "hộp đen" của các mô hình học sâu bằng cách không chỉ cung cấp kết quả phân loại mà còn diễn giải nguyên nhân thông qua việc phân tích từ khóa, mật độ viết hoa, cảm xúc (sentiment) và tính chủ quan của văn bản.

Kết quả thực nghiệm cho thấy mô hình đạt độ chính xác (Accuracy) lên đến \textbf{96.32\%} và F1-Score đạt \textbf{93.42\%}, minh chứng cho tính hiệu quả và độ tin cậy của kiến trúc đề xuất. Hơn thế nữa, ứng dụng được triển khai với kiến trúc Client-Server hiện đại, tích hợp giao diện thân thiện, trực quan, mang lại trải nghiệm người dùng tiện lợi. Đề tài không chỉ đóng góp về mặt kỹ thuật trong lĩnh vực Xử lý Ngôn ngữ Tự nhiên (NLP) mà còn có tiềm năng ứng dụng thực tiễn cao trong việc xây dựng một không gian thông tin số lành mạnh, an toàn và minh bạch tại Việt Nam.
...
Experimental results demonstrate that the model achieves an Accuracy of \textbf{96.32\%} and an F1-Score of \textbf{93.42\%}, proving the effectiveness and reliability of the proposed architecture. Furthermore, the application is deployed with a modern Client-Server architecture and a user-friendly, intuitive interface, providing an optimal user experience. This project not only makes a technical contribution to the field of Natural Language Processing (NLP) but also demonstrates high practical potential in building a healthy, safe, and transparent digital information environment in Vietnam.
...
\renewcommand{\arraystretch}{1.35}\textbf{Từ viết tắt} & \textbf{Giải thích}\\ \hline
...
BERT & Bidirectional Encoder Representations from Transformers (Mô hình biểu diễn ngôn ngữ hai chiều từ Transformer)\\ \hline
...
DOM & Document Object Model (Mô hình Đối tượng Tài liệu)\\ \hline
EDA & Exploratory Data Analysis (Khai phá và Phân tích Dữ liệu)\\ \hline
ERD & Entity-Relationship Diagram (Sơ đồ Thực thể - Mối quan hệ)\\ \hline
...
JWT & JSON Web Token (Mã thông báo Web JSON)\\ \hline
LSTM & Long Short-Term Memory (Bộ nhớ ngắn hạn dài)\\ \hline
ML & Machine Learning (Học máy)\\ \hline
MLOps & Machine Learning Operations (Vận hành Học máy)\\ \hline
...
SeqCls & Sequence Classification (Phân loại chuỗi)\\ \hline
SVM & Support Vector Machine (Máy học véc-tơ hỗ trợ)\\ \hline
TF-IDF & Term Frequency-Inverse Document Frequency (Tần suất xuất hiện của từ - Nghịch đảo tần suất văn bản)\\ \hline
...
VRAM & Video Random Access Memory (Bộ nhớ truy cập ngẫu nhiên đồ họa)\\ \hline
XAI & Explainable Artificial Intelligence (Trí tuệ nhân tạo minh bạch)\\ \hline
\end{tabular}
\end{center}
\newpage
...
\item \textbf{Hệ thống hóa lý thuyết:} Tổng hợp tri thức về NLP, mô hình PhoBERT, phân loại Sequence Classification và giải thích rule-based (XAI).
\item \textbf{Tối ưu mô hình PhoBERT Sequence Classification:} Ứng dụng kỹ thuật tinh chỉnh (Fine-tuning) toàn bộ trọng số của PhoBERT, nạp trực tiếp qua lớp \texttt{AutoModelForSequenceClassification} và sử dụng hàm Softmax để tính xác suất, tinh giản các mô-đun mạng nơ-ron rời rạc.
\item \textbf{Xây dựng Động cơ giải thích (Explanation Engine):} Thiết kế module rule-based độc lập chạy sau khi mô hình dự đoán xong, nhằm sinh ra câu văn tiếng Việt minh bạch hóa lý do bài báo bị cảnh báo.
\item \textbf{Phát triển nền tảng phần mềm nguyên mẫu (prototype):} Thiết kế và triển khai kiến trúc Client-Server chuyên nghiệp để biến mô hình AI nghiên cứu thành sản phẩm thực tiễn (ShieldAI), mang lại trải nghiệm tương tác liền mạch cho người dùng đầu cuối.
\item \textbf{Đánh giá mô hình:} So sánh hiệu năng mô hình đề xuất với các thuật toán truyền thống (Decision Tree, Naive Bayes).
\end{itemize}

\secA{1.3. Đối tượng và phạm vi nghiên cứu}
\secB{1.3.1. Đối tượng nghiên cứu}
Bao gồm: (1) Mô hình ngôn ngữ lớn PhoBERT được tinh chỉnh; (2) Pipeline tiền xử lý \texttt{preprocess\_text}; (3) Giải thích rule-based (\texttt{ExplanationEngine}); và (4) Công nghệ Web (FastAPI, Next.js, SQLite).
...
\item \textbf{Cấu trúc Triển khai ứng dụng (Deployment):} Tiểu luận được hệ thống hóa theo kiến trúc Client-Server chuyên nghiệp. Giao diện người dùng (Frontend) được phát triển bằng Next.js 14 mang lại trải nghiệm tương tác mượt mà; Máy chủ (Backend) sử dụng FastAPI bất đồng bộ cho hiệu năng cao; Dữ liệu lịch sử quét tin giả được lưu trữ an toàn bằng SQLite. Đặc biệt, hệ thống tích hợp Trình thu thập dữ liệu (Web Crawler) bằng BeautifulSoup giúp tự động cào nội dung bài báo trực tiếp từ URL, cải thiện đáng kể trải nghiệm người dùng (UX) thay vì ép người dùng dán văn bản thuần túy.
\end{itemize}

\secA{1.4. Phương pháp nghiên cứu}
Để giải quyết toàn diện bài toán phát hiện tin giả và xây dựng một ứng dụng tích hợp, tiểu luận áp dụng kết hợp các phương pháp nghiên cứu sau:
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
...
\item \textbf{Phương pháp kỹ nghệ phần mềm (Software Engineering):} Áp dụng quy trình phát triển hiện đại để thiết kế kiến trúc phân tán 3 tầng (Client-Server), lập trình giao diện người dùng (Next.js) và tích hợp API máy chủ (FastAPI), qua đó hiện thực hóa mô hình AI thành một phần mềm thực tiễn.
\end{itemize}

\secA{1.5. Ý nghĩa khoa học và thực tiễn}
\textbf{Về mặt khoa học (Tính mới của đề tài):} Thay vì đi theo lối mòn cố gắng thiết kế một thuật toán học sâu hoàn toàn mới từ số không (thường gặp rủi ro về độ ổn định), tính mới cốt lõi của tiểu luận nằm ở \textbf{Đổi mới Kiến trúc Tích hợp (Architectural Innovation)}. Cụ thể, tiểu luận đề xuất và thiết kế thành công một \textbf{"Kiến trúc Phân tách Ngữ nghĩa - Kinh nghiệm" (Decoupled Semantic-Heuristic Architecture)}. Trong kiến trúc này, việc phân loại ngữ nghĩa sâu được đảm nhiệm chính bởi PhoBERT, trong khi việc trích xuất lý do minh bạch được cô lập thành một Động cơ Giải thích bằng luật (Rule-based Explanation) chạy song song. Sự phân tách (Decoupling) này giải quyết được một bài toán hóc búa trong thực tiễn: Nó mang lại khả năng giải thích (XAI) cho mô hình học sâu mà không làm tăng độ trễ suy luận (Inference Latency) — một nhược điểm chí mạng của các thuật toán XAI nội tại (như SHAP hay LIME) khi triển khai trên hệ thống Web thời gian thực.

\textbf{Về mặt thực tiễn:} Đề tài đã đóng gói toàn bộ các tiến bộ khoa học kể trên thành nền tảng phần mềm ShieldAI hoàn thiện. Đây là một công cụ hỗ trợ kiểm chứng thông tin, cung cấp cho cộng đồng mạng Việt Nam khả năng nhận diện tin giả nhanh chóng kèm theo những lời giải thích dễ hiểu. Bên cạnh đó, với kiến trúc API linh hoạt, hệ thống có tiềm năng rất lớn để mở rộng và tích hợp vào các mạng xã hội hoặc các công cụ kiểm duyệt nội dung tự động của các cơ quan báo chí trong tương lai.

\secA{1.6. Bố cục tiểu luận}
Ngoài phần Lời mở đầu và Kết luận, nội dung chính của tiểu luận được tổ chức thành 5 chương cốt lõi như sau:
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
...
\item \textbf{Chương 4 - Kết quả thực nghiệm:} Công bố các số liệu huấn luyện mô hình, kết quả kiểm thử hệ thống tự động (Unit Test) và minh họa giao diện sử dụng thực tế của phần mềm ShieldAI.
...
Để xây dựng một kiến trúc hệ thống phân tích tin giả phức tạp, việc nắm bắt các khái niệm cốt lõi mang tính chất nền tảng là điều kiện tiên quyết. Mục này sẽ trình bày và định nghĩa một cách khoa học các thuật ngữ chuyên ngành được sử dụng xuyên suốt trong tiểu luận.

\secB{2.1.1. Tin giả (Fake News) [3, 4] và Thông tin sai lệch}
Trong kỷ nguyên số, \textbf{Tin giả (Fake News) [3, 4]} được định nghĩa là những thông tin sai sự thật, bị bóp méo, hoặc bịa đặt hoàn toàn, được trình bày dưới hình thức tin báo chí nhằm mục đích lừa dối người đọc. Điểm khác biệt mấu chốt của tin giả so với các lỗi sai sót báo chí thông thường nằm ở \textbf{động cơ (intent)}. Tin giả được tạo ra một cách có chủ đích nhằm thu lợi ích tài chính (thông qua việc thu hút lượt nhấp chuột - clickbait để lấy doanh thu quảng cáo) hoặc phục vụ các mục đích chính trị, thao túng tâm lý và định hướng dư luận xã hội.
...
Trong thực tiễn kiểm chứng thông tin (Fact-checking), việc xác định một văn bản có phải tin giả hay không hiếm khi chỉ dựa vào một yếu tố đơn lẻ, mà đòi hỏi sự đánh giá đa chiều qua nhiều khía cạnh trọng yếu. Dưới đây là các tiêu chí cốt lõi thường được sử dụng để phân loại và nhận diện tin giả:
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
...
\item \textbf{Mục đích kích động cảm xúc (Emotional Manipulation):} Các nội dung này được thiết kế có chủ đích nhằm khơi gợi các phản ứng cảm xúc cực đoan (sợ hãi, phẫn nộ, hoang mang) để thúc đẩy người đọc chia sẻ nhanh chóng thay vì dừng lại kiểm chứng.
...
\item \textbf{Đối chiếu chéo và mức độ lan truyền (Cross-verification \& Spread):} Khi đối chiếu với các nền tảng báo chí lớn khác thì không có thông tin xác nhận sự kiện đó, trong khi trên mạng xã hội lại lan truyền với tốc độ chóng mặt, thường do các tài khoản ảo (bot) phát tán.
\end{itemize}

Dựa trên các tiêu chí nhận diện thực tiễn này, hệ thống ShieldAI được thiết kế nhằm tự động hóa quá trình kiểm duyệt, trong đó đặc biệt tập trung sức mạnh tính toán để tập trung khắc phục khía cạnh phân tích ngôn ngữ, văn phong thao túng và cảm xúc thông qua kiến trúc học sâu PhoBERT kết hợp với cơ chế Heuristics độc lập.
...
Để đo lường một cách khách quan năng lực phân loại của hệ thống, tiểu luận sử dụng các thang đo chuẩn quốc tế dựa trên \textbf{Ma trận nhầm lẫn (Confusion Matrix)}. Ma trận này phân tách kết quả thành 4 nhóm: \textit{True Positive - TP} (Hệ thống bắt đúng tin giả), \textit{True Negative - TN} (Hệ thống nhận diện đúng tin thật), \textit{False Positive - FP} (Hệ thống nhận diện nhầm tin thật thành tin giả), và \textit{False Negative - FN} (Hệ thống bỏ sót tin giả). Từ ma trận này, tiểu luận sử dụng 4 chỉ số cốt lõi:

\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
\item \textbf{Độ chính xác tổng thể (Accuracy):} Là tỷ lệ phần trăm số bài báo (cả thật và giả) được hệ thống phân loại đúng trên tổng số các bài báo trong tập kiểm thử. Tuy nhiên, trong môi trường thực tế (nơi lượng tin thật thường áp đảo tin giả), Accuracy không phải là thước đo duy nhất đáng tin cậy.
\item \textbf{Độ chuẩn xác (Precision):} Trả lời cho câu hỏi: \textit{"Trong số những bài báo mà hệ thống dán nhãn là tin giả, có bao nhiêu phần trăm thực sự là tin giả?"}. Chỉ số này đặc biệt quan trọng để hệ thống giảm thiểu tối đa \textit{False Positive} (ngăn chặn việc vu khống hoặc kiểm duyệt nhầm các bài báo chân chính).
...
\item \textbf{Điểm F1 (F1-Score):} Là trung bình điều hòa (Harmonic Mean) giữa Precision và Recall. Đây là chỉ số tối thượng (Ultimate Metric) được tiểu luận sử dụng làm tiêu chuẩn chính để quyết định mô hình nào hoạt động tốt nhất. F1-Score đảm bảo sự cân bằng hiệu quả, buộc hệ thống vừa không được phép "bắt nhầm" (Precision cao) vừa không được "bỏ sót" (Recall cao).
\end{itemize}

\secA{2.2. Cơ sở lý thuyết chuyên sâu}
Nếu mục 2.1 dừng lại ở việc định nghĩa các khái niệm nền tảng, thì mục này sẽ đi sâu vào việc mổ xẻ cấu trúc toán học và logic vận hành bên trong của các thành phần cốt lõi cấu thành nên hệ thống AI (PhoBERT Fine-tuned Sequence Classification).

\secB{2.2.1. Kiến trúc Transformer và Cơ chế Tự chú ý (Self-Attention)}
Trước khi kiến trúc Transformer [6, 7] ra đời (được giới thiệu bởi Vaswani et al., 2017), các bài toán NLP chủ yếu dựa vào mạng nơ-ron hồi quy (RNN) hoặc bộ nhớ ngắn hạn dài (LSTM). Hạn chế lớn nhất của RNN/LSTM là tính toán tuần tự, dẫn đến hiện tượng "quên" ngữ cảnh đối với các câu văn dài và không thể tính toán song song. Transformer ra đời đã khắc phục hạn chế này bằng cách thay thế cấu trúc hồi quy, thay vào đó sử dụng độc quyền cơ chế \textbf{Tự chú ý (Self-Attention)}.
...
\textbf{PhoBERT} là một mô hình ngôn ngữ tiền huấn luyện tiên phong được huấn luyện dành riêng cho tiếng Việt. Về mặt kiến trúc cốt lõi, PhoBERT không kế thừa nguyên bản BERT của Google, mà dựa trên biến thể được tối ưu hóa mạnh mẽ hơn là \textbf{RoBERTa (Robustly Optimized BERT Pretraining Approach)}.

Khác biệt chuyên sâu của PhoBERT nằm ở chiến lược huấn luyện. Trong khi BERT sử dụng kỹ thuật \textit{Masked Language Model (MLM)} tĩnh (các từ bị che giấu cố định trong suốt quá trình huấn luyện), PhoBERT áp dụng \textbf{MLM động (Dynamic Masking)}. Nghĩa là, ở mỗi chu kỳ huấn luyện (epoch), các từ bị che khuất sẽ thay đổi liên tục một cách ngẫu nhiên. Điều này buộc mạng nơ-ron phải học cách dự đoán từ còn thiếu dựa trên các cấp độ từ vựng ghép của tiếng Việt. Được tiền huấn luyện trên khối dữ liệu 20GB, PhoBERT sở hữu khả năng nắm bắt cấu trúc ngữ pháp, từ ghép và ngữ cảnh phức tạp của tiếng Việt.
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
\item \textit{ReINTEL Challenge 2020: Vietnamese Fake News Detection using Ensemble Model with PhoBERT embeddings} (Nhiều tác giả, VLSP 2020): \textbf{Ưu điểm:} Ứng dụng thành công PhoBERT để trích xuất đặc trưng, kết hợp thuật toán Ensemble (StackNet, LightGBM) để đạt F1-Score 94\%. \textbf{Nhược điểm:} Thiếu cơ chế giải thích, chỉ hoạt động dưới dạng mã nguồn thô không có giao diện tương tác.
\item \textit{Phát hiện tự động tin giả: Thành tựu và thách thức} (Tạp chí Khoa học \& Công nghệ - ĐH Đà Nẵng, 2021): \textbf{Ưu điểm:} Hệ thống hóa toàn diện các phương pháp học máy (BoW, SVM) và học sâu (CNN, LSTM) cho ngữ liệu tiếng Việt. \textbf{Nhược điểm:} Chưa triển khai và đánh giá được hiệu năng thực tế của các mô hình Transformer tiên tiến.
\item \textit{Fake News Detection using PhoBERT and BiLSTM with Handcrafted Features} (Nhóm nghiên cứu, 2022): \textbf{Ưu điểm:} Lai ghép thành công PhoBERT và BiLSTM, tận dụng thêm đặc trưng thời gian và độ dài văn bản để tăng độ chuẩn xác. \textbf{Nhược điểm:} Cấu trúc lai phức tạp làm tăng thời gian suy luận (inference time), không phù hợp xử lý tin tức theo thời gian thực.
\item \textit{Nhận diện thông tin sai lệch đa phương thức trên mạng xã hội tiếng Việt} (VLSP ReINTEL): \textbf{Ưu điểm:} Mở ra hướng đi phát hiện tin giả dạng ảnh chế (meme) bằng cách phân tích song song văn bản và hình ảnh. \textbf{Nhược điểm:} Đòi hỏi tài nguyên phần cứng (GPU) cực kỳ lớn, khó tích hợp và vận hành liên tục cho người dùng phổ thông.
\end{itemize}

\secB{2.3.2. Các nghiên cứu ngoài nước}
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
\item \textit{Fake News Detection on Social Media: A Data Mining Perspective} (Kai Shu et al., 2017): Đã phân tách bộ đặc trưng thành Đặc trưng nội dung và Đặc trưng mạng xã hội. \textbf{Ưu điểm:} Đặt nền móng vững chắc cho việc khai thác Heuristics trong tin giả. \textbf{Nhược điểm:} Phương pháp chủ yếu dựa trên các bộ đếm thủ công, bỏ qua việc phân tích ngữ nghĩa sâu của ngữ cảnh.
\item \textit{Emotion shapes the diffusion of moralized content} (Brady et al., Tạp chí PNAS, 2017): Chứng minh bằng thực nghiệm rằng từ vựng phẫn nộ làm tăng tốc độ lan truyền thông tin. \textbf{Ưu điểm:} Cung cấp cơ sở lý thuyết tâm lý vững chắc cho tính năng Phân tích Cảm xúc (Sentiment Analysis). \textbf{Nhược điểm:} Chỉ phân tích khía cạnh tâm lý xã hội học, chưa đề xuất một kiến trúc trí tuệ nhân tạo cụ thể để tự động phát hiện.
\item \textit{dEFEND: Explainable Fake News Detection} (Kai Shu et al., Hội nghị KDD 2019): Xây dựng mô hình phân loại kết hợp cơ chế Attention để bôi đậm (highlight) các câu văn bất thường. \textbf{Ưu điểm:} Tiên phong trong việc hiện thực hóa tính minh bạch (Explainable AI) cho bài toán tin giả. \textbf{Nhược điểm:} Khả năng giải thích còn phụ thuộc vào trọng số mạng nơ-ron thay vì các luật ngôn ngữ tường minh, khó cấu hình cho ngôn ngữ tiếng Việt.
\item \textit{FakeBERT: Fake news detection in social media with a BERT-based deep learning approach} (Kaliyar et al., Multimedia Tools and Applications, 2020): Sử dụng mạng 1D-CNN kết hợp với BERT. \textbf{Ưu điểm:} Đạt độ chính xác rất cao (>98\%) trên các bài báo tiếng Anh dài nhờ sự kết hợp giữa trích xuất đặc trưng cục bộ và toàn cục. \textbf{Nhược điểm:} Tốc độ suy luận rất chậm, hệ thống hoạt động theo cơ chế "hộp đen" (Black-box).
\item \textit{Fake News Detection Using BERT Model with Handcrafted Features} (IEEE, 2021): Tích hợp trực tiếp vector ngữ nghĩa từ BERT với ma trận đặc trưng thủ công (số chữ in hoa, dấu câu). \textbf{Ưu điểm:} Tăng cường đáng kể độ chuẩn xác phân loại nhị phân. \textbf{Nhược điểm:} Việc dung hợp vector thủ công và vector Transformer trực tiếp làm tăng số lượng tham số tính toán và dễ dẫn đến hiện tượng học vẹt (overfitting).
\end{itemize}

\secB{2.3.3. So sánh và làm nổi bật tính ưu việt của tiểu luận (ShieldAI)}
Dựa trên bức tranh tổng quan của các nghiên cứu đi trước, Bảng \ref{tab:so_sanh_nghien_cuu} dưới đây tổng hợp và so sánh trực tiếp các hệ thống nhằm xác định vị trí và ưu điểm của tiểu luận \textbf{ShieldAI}:

\begin{table}[H]
\centering
\small
\caption{Bảng so sánh các công trình nghiên cứu tiêu biểu}
\label{tab:so_sanh_nghien_cuu}
\begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{0.5cm}|>{\raggedright\arraybackslash}p{3cm}|>{\raggedright\arraybackslash}p{3cm}|X|X|}
\hline
\textbf{STT} & \textbf{Công trình} & \textbf{Phương pháp} & \textbf{Ưu điểm cốt lõi} & \textbf{Nhược điểm / Hạn chế} \\
\hline
1 & Shu et al. (2017) & Khai phá dữ liệu \& Heuristics & Hệ thống hóa các bộ đặc trưng chuẩn. & Bỏ qua phân tích ngữ nghĩa sâu. \\
\hline
2 & VLSP ReINTEL (2020) & PhoBERT + Ensemble Learning & Tối ưu hóa rất tốt cho ngôn ngữ tiếng Việt. & Mô hình hộp đen (Black-box), chi phí tính toán cao. \\
\hline
3 & FakeBERT (2020) & 1D-CNN + BERT & Độ chính xác rất cao trên ngữ liệu tiếng Anh. & Không có cơ chế giải thích minh bạch. \\
\hline
4 & IEEE Paper (2021) & BERT + Handcrafted Features & Nâng cao hiệu năng nhờ lai ghép vector ngữ nghĩa. & Tốc độ suy luận chậm, dễ bị học vẹt. \\
\hline
\textbf{5} & \textbf{ShieldAI (Đề xuất)} & \textbf{PhoBERT Fine-tuned + Rule-based XAI} & \textbf{Độ chính xác cao, tích hợp hệ thống giải thích độc lập, giao diện thực tiễn.} & \textbf{Chưa hỗ trợ phân tích đa phương thức (ảnh/video).} \\
\hline
\end{tabularx}
\end{table}

Qua phân tích từ Bảng \ref{tab:so_sanh_nghien_cuu}, có thể thấy giải pháp đề xuất \textbf{ShieldAI} tập trung khắc phục vấn đề "Hộp đen" của các mô hình học sâu hiện đại (như FakeBERT hay hệ thống VLSP 2020) bằng cách phát triển Động cơ Giải thích bằng luật (Rule-based XAI) chạy song song. Hơn nữa, việc loại bỏ các mạng nơ-ron phức tạp ở tầng cuối (giống như IEEE 2021) và chỉ sử dụng cấu trúc \textit{PhoBERT Sequence Classification} đã giúp hệ thống đạt độ cân bằng tương đối tốt giữa độ chính xác phân loại và tốc độ phản hồi trên môi trường thực tế.

\secA{2.4. Công nghệ và Công cụ sử dụng}
Tiểu luận này không chỉ dừng lại ở việc thiết kế một mô hình AI chạy trong môi trường nghiên cứu cô lập (Jupyter Notebook), mà được xây dựng thành một nền tảng phần mềm nguyên mẫu (prototype) theo kiến trúc Client-Server. Dưới đây là các công nghệ lõi được sử dụng để hiện thực hóa toàn bộ hệ thống:
...
Động cơ này hoạt động bằng cách đo lường trực tiếp các tập đặc trưng Heuristics trên văn bản thô, bao gồm các chỉ số như tỷ lệ chữ in hoa, mật độ dấu câu, độ dài tiêu đề. Dựa trên các tập luật (rules) được lập trình sẵn, nó sẽ tự động biên dịch các chỉ số bất thường này thành những câu văn tiếng Việt thân thiện (ví dụ: "Lạm dụng dấu câu quá mức"). Các câu văn giải thích này được hiển thị đồng thời cùng với dự đoán xác suất, giúp minh bạch hóa lý do cảnh báo tin giả.
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    ...
    \item \textbf{Sự đa dạng của Không gian Đặc trưng (Feature Space):} Dữ liệu đến từ nhiều nền tảng cấu trúc khác nhau (báo chí tiêu chuẩn và mạng xã hội). Do đó, dữ liệu đầu vào chứa vô số các "nhiễu" tự nhiên như biểu tượng cảm xúc (emoji), từ lóng, và các chiến thuật thao túng. Đây chính là môi trường huấn luyện hoàn hảo (Robustness Training) để Động cơ Giải thích bằng luật (Heuristic Engine) trích xuất tối đa các đặc trưng tâm lý.
...
Do quy mô dữ liệu khổng lồ, tập dữ liệu được phân chia theo tỷ lệ \textbf{76\% (Train) - 12\% (Validation) - 12\% (Test)} để phục vụ cho các vòng đời huấn luyện và đánh giá của mô hình AI. Đặc biệt, kỹ thuật phân tầng (Stratified Split) được áp dụng nghiêm ngặt thông qua hàm \texttt{train\_test\_split} của thư viện Scikit-learn. Quá trình phân chia này được thực hiện \textbf{trước} khi áp dụng bất kỳ bước tiền xử lý, gán nhãn hay gia tăng dữ liệu (Data Augmentation) nào. Việc cách ly tuyệt đối tập kiểm thử (12\% dữ liệu, tương đương hơn 2.700 mẫu) khỏi tập huấn luyện ngay từ ban đầu (Data split before Vectorization) là cơ chế phòng vệ cốt lõi để loại trừ hiện tượng Rò rỉ dữ liệu (Data Leakage) ở cấp độ từ vựng, đảm bảo độ tin cậy thực sự của các chỉ số hiệu năng.

\secB{3.1.4. Phương pháp Trích xuất biểu diễn ngữ nghĩa bằng PhoBERT}
Tiểu luận sử dụng mô hình ngôn ngữ tiền huấn luyện PhoBERT (dựa trên kiến trúc Transformer) làm thành phần cốt lõi để trích xuất đặc trưng văn bản. Văn bản tiếng Việt sau khi làm sạch sẽ được đưa qua bộ Tokenizer của PhoBERT để chuyển đổi thành chuỗi các sub-words. Hệ thống trích xuất vector nhúng (embedding) ở vị trí token đặc biệt \texttt{[CLS]} tại lớp ẩn cuối cùng (last hidden state). Vector 768 chiều này chứa đựng biểu diễn ngữ nghĩa sâu sắc của toàn bộ đoạn văn bản, bao gồm cả cấu trúc ngữ pháp và sự tương quan ngữ cảnh đa chiều giữa các từ. Đây là nguồn dữ liệu duy nhất và giàu thông tin nhất được sử dụng làm đầu vào cho bộ phân loại Học máy lõi, đảm bảo mô hình không bị nhiễu bởi các yếu tố thủ công.
...
Về mặt đánh giá, hệ thống áp dụng phương pháp đánh giá trên tập kiểm thử độc lập (Hold-out Test Set) và sử dụng chỉ số F1-Score làm thước đo cốt lõi. Đề tài tích hợp \textbf{ExplanationEngine} dựa trên luật (rule-based) để bóc tách các dấu hiệu rủi ro (viết hoa, dấu câu, độ dài tiêu đề). Cần lưu ý rằng cơ chế XAI hiện tại chỉ dừng ở mức giải thích bề mặt (Post-hoc Global Explanation) chứ chưa giải mã được nội tại cơ chế Attention của PhoBERT thông qua các kỹ thuật tiên tiến như SHAP hay LIME. Tuy nhiên, sự đánh đổi này là cần thiết trong phiên bản hiện tại để giảm thiểu độ phức tạp tính toán và đáp ứng tốc độ phản hồi của API.
...
Ở tầng xử lý trung tâm (Backend), hệ thống sử dụng khung làm việc FastAPI [12] dựa trên ngôn ngữ Python. Theo thực nghiệm của Lantz (2020) [16], kiến trúc xử lý bất đồng bộ (Asynchronous) của FastAPI giúp giải quyết hiệu quả hiện tượng thắt cổ chai (bottleneck) khi phải điều phối song song các truy vấn đến mô hình ngôn ngữ lớn như PhoBERT. Dữ liệu người dùng và lịch sử phân tích được chuẩn hóa và lưu trữ an toàn thông qua cơ chế ánh xạ đối tượng quan hệ (SQLAlchemy ORM) [17], giúp cô lập cơ sở dữ liệu khỏi các rủi ro bảo mật từ các truy vấn SQL thô.
...
Dựa trên mục tiêu nghiên cứu và bối cảnh ứng dụng thực tiễn, hệ thống phát hiện tin giả được thiết kế với các yêu cầu chức năng cốt lõi (được trình bày chi tiết tại Bảng \ref{tab:yeu_cau_chuc_nang}). Hệ thống được phân rã thành 5 phân hệ (Module) chính nhằm đảm bảo luồng xử lý dữ liệu thông suốt từ phía người dùng đến tầng học sâu. Bảng \ref{tab:yeu_cau_chuc_nang} tóm tắt các yêu cầu chức năng chủ chốt, trong đó yêu cầu FR-05 và FR-06 đóng vai trò cốt lõi thể hiện năng lực phân loại của mô hình trí tuệ nhân tạo, đồng thời tích hợp Động cơ Giải thích để đảm bảo tính minh bạch của dự đoán.
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    \item \textbf{Hiệu năng và Tính chịu tải (Performance \& Concurrency):} Kiến trúc xử lý bất đồng bộ của tầng Backend yêu cầu khả năng điều phối song song (Concurrency control) để hạn chế tối đa độ trễ khi mô hình nạp hàng triệu tham số vào bộ nhớ. Khung làm việc FastAPI hỗ trợ xử lý hàng ngàn luồng kết nối đồng thời mà không tạo ra hiện tượng chặn (blocking).
    ...
    \item \textbf{Tính module hóa và khả năng MLOps (Maintainability):} Mô hình học sâu PhoBERT sau khi huấn luyện được đóng gói dưới dạng \textit{Artifacts} riêng biệt, tách rời khỏi mã nguồn điều phối (API). Thiết kế này tuân thủ nguyên tắc Tách biệt Mối quan tâm (Separation of Concerns), cho phép các nhà nghiên cứu dễ dàng tái huấn luyện và cập nhật trọng số mạng nơ-ron mà không gây đứt gãy luồng vận hành của máy chủ hệ thống [21].
\end{itemize}

\secA{3.4. Thiết kế hệ thống}
\secB{3.4.1. Kiến trúc hệ thống}
Cấu trúc hệ thống (System Architecture) là cách các thành phần của một hệ thống được tổ chức, sắp xếp và liên kết với nhau để hệ thống hoạt động và đạt được mục tiêu đề ra. Dựa trên định nghĩa cốt lõi đó và các yêu cầu đã phân tích, dự án ShieldAI được thiết kế theo kiến trúc phân tán \textbf{Client-Server (Front-end \& Back-end Separation)} với mô hình 3 tầng (3-Tier) độc lập. Việc tách biệt các tầng giúp tối đa hóa khả năng bảo trì mã nguồn, tăng tốc độ truy vấn cơ sở dữ liệu và tối ưu hóa tài nguyên phần cứng khi xử lý các mô hình học sâu. Cụ thể, cách các thành phần của hệ thống được tổ chức và liên kết với nhau được minh họa trực quan trong sơ đồ cấu trúc dưới đây:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/kientruc.png}
    \caption{Sơ đồ Kiến trúc Hệ thống Phân tán}
    \label{fig:kien_truc}
\end{figure}

Như minh họa trong sơ đồ trên, thay vì gộp chung mọi logic xử lý vào một khối nguyên khối (Monolith Architecture), tiểu luận đã áp dụng kiến trúc phân tán \textbf{3 tầng (3-Tier Micro-Architecture) độc lập} (cùng với 1 tầng lưu trữ). Việc chia tách này không chỉ giúp quản lý mã nguồn dễ dàng mà còn đảm bảo tính mở rộng (Scalability), tính tái sử dụng và bảo mật (Security) cho hệ thống trong môi trường thực tế. Sự giao tiếp giữa các tầng được kiểm soát chặt chẽ thông qua các chuẩn HTTP và dữ liệu JSON. Cụ thể, các tầng được thiết kế chi tiết như sau:

\textbf{Tầng 1: Tầng Giao diện Người dùng (Presentation / Client Tier)}

Tầng Giao diện đóng vai trò là điểm chạm đầu tiên (touchpoint) giữa người dùng và hệ thống. Được xây dựng hoàn toàn bằng \textbf{Next.js 14} (một React Framework hiện đại) kết hợp cùng \textbf{Tailwind CSS} và thư viện chuyển động \textbf{Framer Motion}, tầng này đảm bảo trải nghiệm đa nền tảng (Responsive Design) và tương tác ổn định từ màn hình máy tính đến thiết bị di động. Điểm cốt lõi trong thiết kế của Tầng Giao diện là nó hoạt động độc lập (agnostic) với các thuật toán Trí tuệ Nhân tạo phức tạp bên dưới. Nhiệm vụ của nó chỉ là tiếp nhận yêu cầu đầu vào từ người dùng (một đoạn văn bản nghi ngờ hoặc một đường dẫn URL bài báo), đóng gói thành các truy vấn chuẩn HTTP RESTful, và gửi đi. Khi nhận được kết quả trả về từ máy chủ, module \textbf{Trực quan hóa Dữ liệu} sẽ sử dụng các công cụ vẽ biểu đồ linh hoạt để kết xuất các thông số đánh giá rõ ràng (ví dụ: thanh tiến trình hiển thị 99\% giả mạo, biểu đồ radar cho các chỉ số cảm xúc, hoặc bôi đỏ các từ ngữ giật gân). Cơ chế này giúp người dùng phổ thông, dù không có chuyên môn về máy học, vẫn có thể dễ dàng nắm bắt được kết quả phân loại.
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    \item \textbf{Bảng USERS (Quản lý Tài khoản):} Chịu trách nhiệm lưu trữ thông tin cá nhân. Trường \texttt{email} và \texttt{username} được đánh chỉ mục độc nhất (Unique Index) để làm định danh đăng nhập. Mật khẩu không được lưu dưới dạng văn bản thô, mà sử dụng thuật toán băm một chiều để tạo ra trường \texttt{hashed\_password} nhằm ngăn chặn rò rỉ dữ liệu.
    \item \textbf{Bảng ANALYSIS\_HISTORY (Lịch sử Phân tích):} Đây là quyển "Sổ cái" ghi lại chi tiết hoạt động của Động cơ AI. Bên cạnh các dữ liệu đầu vào như \texttt{input\_mode} (URL/Text), \texttt{title}, \texttt{content}, bảng còn lưu trữ chi tiết phán quyết của AI thông qua \texttt{fake\_prob} (xác suất) và \texttt{verdict} (3 ngưỡng cảnh báo). Đặc biệt, trường \texttt{explanation\_json} và \texttt{meta\_json} cho phép linh hoạt lưu trữ báo cáo Heuristics bán cấu trúc (NoSQL-like) ngay bên trong CSDL quan hệ.
    \item \textbf{Bảng FEEDBACKS (Quản lý Phản hồi):} Đóng vai trò quan trọng trong chiến lược MLOps (Machine Learning Operations). Bảng cho phép người dùng đánh giá tính chính xác của phán quyết AI (\texttt{is\_correct}) và để lại bình luận (\texttt{comment}). Dữ liệu này là nguồn tài nguyên quý giá để nhóm nghiên cứu tái huấn luyện (Re-train) mô hình trong tương lai.
\end{itemize}
...
\hline
\textbf{USERS} & \texttt{email} & String(255) & Email đăng nhập (Unique, Index). \\
\hline
\textbf{USERS} & \texttt{username} & String(64) & Tên người dùng hiển thị (Unique, Index). \\
\hline
\textbf{USERS} & \texttt{hashed\_password} & String(255) & Mật khẩu đã mã hóa. \\
\hline
...
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{user\_id} & Integer & Khóa ngoại (FK) trỏ đến bảng USERS. \\
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{input\_mode} & String(10) & Loại đầu vào: "text" hoặc "url". \\
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{title} & String(500) & Tiêu đề bài báo (nếu có). \\
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{content} & Text & Nội dung bài báo hoặc đoạn văn bản. \\
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{source\_url} & String(2048) & URL gốc của bài báo (nếu input\_mode="url"). \\
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{source\_domain} & String(255) & Tên miền gốc của URL (nếu có). \\
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{result\_label} & String(64) & Nhãn dự đoán cuối cùng. \\
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{fake\_prob} & Float & Xác suất giả mạo từ AI (0.0 đến 1.0). \\
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{verdict} & String(32) & Ngưỡng cảnh báo (Tin Thật/Đáng ngờ/Tin Giả). \\
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{explanation\_json} & Text & Báo cáo giải thích minh bạch từ Heuristics. \\
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{meta\_json} & Text & Thông tin siêu dữ liệu mở rộng. \\
\hline
\textbf{ANALYSIS\_HISTORY} & \texttt{created\_at} & DateTime & Thời gian phân tích. \\
\hline
\textbf{FEEDBACKS} & \texttt{id} & Integer & Khóa chính (PK), tự động tăng. \\
\hline
\textbf{FEEDBACKS} & \texttt{history\_id} & Integer & Khóa ngoại (FK) trỏ 1-1 về bảng ANALYSIS\_HISTORY. \\
\hline
\textbf{FEEDBACKS} & \texttt{user\_id} & Integer & Khóa ngoại (FK) trỏ về bảng USERS. \\
\hline
\textbf{FEEDBACKS} & \texttt{is\_correct} & Boolean & Đánh giá dự đoán có chính xác không (Thumbs up/down). \\
\hline
\textbf{FEEDBACKS} & \texttt{comment} & Text & Nhận xét chi tiết từ người dùng (nếu có). \\
\hline
\textbf{FEEDBACKS} & \texttt{created\_at} & DateTime & Thời gian gửi phản hồi. \\
\hline
\end{tabular}
}
\end{table}

Mối quan hệ giữa các thực thể này là mối quan hệ \textbf{1-N (Một - Nhiều)} giữa người dùng với lịch sử phân tích, và quan hệ \textbf{1-1 (Một - Một)} giữa bản phân tích và một phản hồi duy nhất. Việc ứng dụng ORM SQLAlchemy giúp liên kết tường minh thông qua các khóa ngoại (Foreign Keys). Khái niệm này phản ánh luồng nghiệp vụ thực tế: Một người dùng quét nhiều bài báo, họ đánh giá các báo cáo này, và toàn bộ dữ liệu trở thành quy mô tri thức mới khổng lồ để cải thiện AI trong tương lai.

\secB{3.4.3. Thiết kế giao diện (UI/UX Design) và Trải nghiệm Người dùng}
Với mục tiêu đưa công nghệ Trí tuệ Nhân tạo phức tạp đến gần hơn với người dùng phổ thông, Giao diện người dùng (User Interface - UI) và Trải nghiệm người dùng (User Experience - UX) của hệ thống được thiết kế theo triết lý \textbf{Tối giản và Hiện đại (Modern Minimalism)}. Toàn bộ nền tảng Frontend được xây dựng bằng Next.js kết hợp với bộ công cụ \textbf{Tailwind CSS}. Việc ứng dụng Tailwind giúp hệ thống duy trì được một cấu trúc mã màu (Color Palette) nhất quán, áp dụng chuẩn thiết kế bo góc (border-radius mềm mại), hiệu ứng kính mờ (Glassmorphism), và đặc biệt là khả năng tương thích hiển thị hiệu quả trên mọi kích thước màn hình (Fully Responsive) từ màn hình Desktop rộng lớn cho đến điện thoại di động. Tổng thể giao diện hệ thống bao gồm ba phân hệ màn hình cốt lõi, mỗi phân hệ được thiết kế để giải quyết một điểm chạm cụ thể của người dùng:

\textbf{1. Màn hình Cổng thông tin và Phân tích (Home \& Analysis Portal)}

Đây là màn hình đầu tiên khi người dùng truy cập vào ứng dụng. Thiết kế tập trung sự chú ý của người dùng vào một \textbf{Thanh tìm kiếm (Search Bar)} kích thước lớn đặt ở chính giữa không gian làm việc. Thanh tìm kiếm này được thiết kế thông minh với tính năng đa định dạng đầu vào (Multi-input format): người dùng có thể dán (paste) trực tiếp một đường dẫn URL của bài báo mạng, hoặc gõ một đoạn văn bản thô bất kỳ vào khung soạn thảo. Ngay phía dưới là nút "Phân tích nội dung" được tích hợp các hiệu ứng vi mô (micro-interactions) phản hồi xúc giác khi rê chuột (hover). Trong quá trình chờ máy chủ chạy các phép toán AI, màn hình sẽ hiển thị các hiệu ứng tải (Loading animations) dạng sóng mượt mà, nhằm cải thiện trải nghiệm người dùng (UX optimization).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/luong_xu_ly.png}
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    ...
    \item \textbf{Bước 2: Giải mã Ngữ nghĩa (PhoBERT):} Văn bản sạch được đưa qua bộ Tokenizer của PhoBERT để băm nhỏ thành các mảnh từ vựng (Sub-words). Sau đó, chuỗi token này đi xuyên qua 12 lớp Mạng Tự chú ý (Self-Attention) của kiến trúc Transformer [6, 7]. Kết quả trả về là một siêu vector đặc trưng đại diện toàn cục cho ngữ cảnh của câu (thường gọi là \texttt{[CLS] token vector}), có kích thước cực lớn lên đến \textbf{768 chiều}. Vector này nắm giữ toàn bộ "ý nghĩa sâu xa" và "giọng điệu mỉa mai" ẩn giấu trong bài báo.
    \item \textbf{Bước 3: Phân loại (Logits + Softmax):} Mô hình \texttt{AutoModelForSequenceClassification} xuất giá trị Logits, sau đó truyền qua hàm \texttt{Softmax} để tính toán xác suất tin giả; module \texttt{verdict.py} quy đổi 3 mức: $<35\%$ tin thật, 35–74\% đáng ngờ, $\ge 75\%$ tin giả.
...
\begin{verbatim}
ALGORITHM: PhoBERT Sequence Classification & Rule-based Explanation
INPUT:
...
\textbf{Thứ hai, về mặt Thiết kế Dữ liệu và Trải nghiệm:} Việc ứng dụng linh hoạt giữa mô hình quan hệ chặt chẽ (SQLite/SQLAlchemy) và định dạng dữ liệu bán cấu trúc (JSON) trong lưu trữ báo cáo Heuristic cho thấy sự linh hoạt về mặt lưu trữ truy vấn. Song song đó, triết lý thiết kế UI/UX hiện đại (Modern Minimalism) với việc thị giác hóa rủi ro qua mã màu góp phần giảm rào cản kỹ thuật, giúp người dùng phổ thông có thể dễ dàng tương tác và nắm bắt các phán quyết của AI.

\textbf{Thứ ba, về mặt Thiết kế Thuật toán Lõi:} Lưu đồ thuật toán tuần tự mô tả pipeline \textbf{PhoBERT Fine-tuned Sequence Classification}: AutoModelForSequenceClassification (Logits) $\rightarrow$ Softmax $\rightarrow$ verdict 3 mức $\rightarrow$ ExplanationEngine rule-based.
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    ...
    \item \textbf{Bộ xử lý Đồ họa (GPU):} Khuyến nghị card \textbf{NVIDIA} hỗ trợ \textbf{CUDA} để tăng tốc suy luận PhoBERT (embedding). Quá trình huấn luyện Fine-tuning PhoBERT đòi hỏi sức mạnh GPU rất lớn để cập nhật trọng số hiệu quả.
\end{itemize}

\secB{4.1.2. Môi trường Hệ điều hành và Phần mềm}
Để tạo ra một nền tảng vận hành mượt mà, hạn chế tối đa các xung đột thư viện (Dependency conflicts) thường gặp trong quá trình biên dịch mô hình AI, tiểu luận đã lựa chọn và chốt cấu hình các công nghệ phần mềm cốt lõi như sau:
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    \item \textbf{Hệ điều hành cốt lõi:} \textbf{Linux (Ubuntu)} — tương thích mã nguồn mở, khởi chạy script điều phối (\texttt{setup.sh}, \texttt{run.sh}) ổn định.
    ...
    \begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
        ...
        \item \textbf{Thư viện AI Lõi:} \texttt{PyTorch} + \texttt{Transformers} (PhoBERT embedding), \texttt{Scikit-learn} (metrics F1/Precision, Confusion Matrix).
    \end{itemize}
    \item \textbf{Hệ sinh thái Frontend:} Được xây dựng trên nền tảng môi trường cục bộ \texttt{Node.js}. Sử dụng ngôn ngữ \texttt{TypeScript} nhằm ép buộc tính an toàn kiểu dữ liệu (Type-safety). Khung thiết kế chính là \texttt{Next.js 14} kết hợp với hệ sinh thái \texttt{React} và bộ thư viện tiện ích \texttt{Tailwind CSS}.
...
Mã nguồn của hệ thống được tổ chức khoa học theo nguyên tắc \textbf{Separation of Concerns (Tách biệt mối quan tâm)}. Thay vì gộp chung toàn bộ logic vào một khối lệnh nguyên khối khó kiểm soát, tiểu luận đã phân rã hệ thống thành các module Python độc lập. Lối kiến trúc này giúp mã nguồn dễ bảo trì, dễ kiểm thử (Unit Test) và dễ dàng nâng cấp thuật toán trong tương lai. Các module lõi được cài đặt như sau:

\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    \item \textbf{Module Thu thập và Tiền xử lý (\texttt{data\_crawler.py}, \texttt{text\_utils.py}):} Khi người dùng nhập URL, \texttt{data\_crawler.py} dùng \texttt{BeautifulSoup} + \texttt{requests} trích title/content từ HTML. Văn bản thu được (hoặc nhập trực tiếp) đi qua \texttt{preprocess\_text} trong \texttt{text\_utils.py}. Helper \texttt{TextCleaner} (HTML/teencode) chỉ dùng trong test, không nằm luồng chính.
    \item \textbf{Module Suy luận Phân loại (\texttt{phobert\_inference.py}):} Tải PhoBERT-base qua \texttt{Transformers} (PyTorch), trích vector [CLS] 768 chiều, suy luận qua \texttt{AutoModelForSequenceClassification} đã lưu (\texttt{Hugging Face Model}). Logits qua \texttt{Softmax} trả xác suất tin giả; \texttt{verdict.py} quy đổi 3 mức cảnh báo.
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    ...
    \item \textbf{Sự phù hợp với phương pháp Heuristics:} Các tin giả trên không gian mạng thường xuyên lặp lại các hành vi thao túng tâm lý (như viết hoa toàn bộ tựa đề để lôi kéo, dùng hàng loạt dấu chấm than để kích động). Do đó, bộ dữ liệu khổng lồ này chính là "đất diễn" hiệu quả để Động cơ Giải thích Heuristics của tiểu luận phát huy tối đa sức mạnh.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/luong_gop_du_lieu.png}
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    ...
    \item \textbf{Huấn luyện Fine-tuning:} Dữ liệu được mã hóa bằng \texttt{AutoTokenizer} của PhoBERT trước khi được nạp vào mô hình \texttt{AutoModelForSequenceClassification}. Quá trình huấn luyện không sử dụng tái lấy mẫu (resampling) do tập dữ liệu đã tương đối cân bằng.
\end{itemize}
...
Mô hình tổ chức này tuân thủ nguyên tắc \textit{Separation of Concerns} (Tách biệt mối quan tâm), giúp đội ngũ phát triển dễ dàng khoanh vùng lỗi, thực hiện kiểm thử độc lập ở Backend mà không ảnh hưởng tới quá trình thiết kế UI/UX ở Frontend.

\secA{4.3. Kết quả đạt được}
\secB{4.3.1. Kết quả giao diện ứng dụng (User Interface Implementation)}
Hệ thống đã được lập trình hoàn thiện bằng Next.js và triển khai thành công trên môi trường cục bộ. Giao diện thực tế hoạt động ổn định, đáp ứng chính xác các yêu cầu về thiết kế UI/UX đã đề ra ở Chương 3. Dưới đây là các minh chứng hình ảnh chụp lại từ hệ thống thực tế đang vận hành:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/1_TrangChu.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/2_TrangDangNhap.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/01_Giao_dien_Nhap_Van_Ban.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/03_Giao_dien_Nhap_Link.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/08_Ket_Qua_Phan_Tich_Tin_Gia_100_Phan_Tram.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/10_Ket_Qua_Phan_Tich_Tin_That_0_Phan_Tram.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/12_Ket_Qua_Tin_That_Giai_Thich.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/9_TrangLichSu.png}
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    \item \textbf{Accuracy (Độ chính xác tổng thể):} Tỷ lệ phần trăm các bài báo (cả thật và giả) được hệ thống phân loại đúng. Tuy nhiên, trong bài toán tin giả, chỉ số này không phản ánh toàn diện bức tranh nếu dữ liệu bị mất cân bằng.
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    \item \textbf{True Positive (TP) = 1.250:} Nhận diện chính xác 1.250 bài tin giả.
    \item \textbf{True Negative (TN) = 1.365:} Nhận diện chính xác 1.365 bài tin thật.
    \item \textbf{False Positive (FP) = 75:} Phân loại nhầm 75 bài tin thật thành tin giả.
    \item \textbf{False Negative (FN) = 26:} Bỏ lọt 26 bài tin giả (phân loại nhầm thành tin thật).
\end{itemize}
Tỷ lệ Âm tính giả (FN) ở mức rất thấp (chỉ 26 mẫu) cho thấy hệ thống được thiết kế với tư duy "phòng vệ chủ động" - tối ưu hóa để không bỏ lọt các tin giả nguy hiểm ảnh hưởng đến cộng đồng. Tỷ lệ Dương tính giả (FP = 75 mẫu) vẫn tồn tại do hệ thống còn nhầm lẫn với các bài báo giật gân chính thống.
...
Dù đạt hiệu năng cực kỳ ấn tượng, mô hình ShieldAI vẫn ghi nhận một số trường hợp dự đoán sai lệch. Quá trình phân tích lỗi trên tập kiểm thử chỉ ra hai nhóm nguyên nhân cốt lõi:
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    ...
    \item \textbf{Âm tính giả (False Negative - Tin giả lọt lưới):} Một số tin lừa đảo tinh vi cố tình sử dụng lối hành văn cực kỳ trung lập, trang trọng, sao chép y hệt văn phong hành chính của cơ quan chức năng nhà nước (ví dụ: các thông báo phạt nguội giao thông giả mạo). Do hệ thống hiện tại tập trung phân tích văn bản thô (Text-only) và chưa được trang bị khả năng đối chiếu sự kiện (Knowledge Graph / Fact-checking), các bài viết lừa đảo mức độ tinh xảo cao này có thể đánh lừa được trọng số của bộ phân loại học sâu.
\end{itemize}
...
Hệ thống Backend được tích hợp bộ kiểm thử \texttt{pytest} trong thư mục \texttt{backend/tests/}. Tổng cộng \textbf{38 kịch bản (test cases)} chạy tự động, gồm kiểm thử đơn vị (unit test) và kiểm thử tích hợp API (integration test). Các test API dùng SQLite in-memory và mock \texttt{PhoBERTInferenceSystem}; lỗi nghiệp vụ (auth, input trống) được assert qua \texttt{status=error} trong body JSON với HTTP 200.
...
Ở tầng giao diện trực quan (Frontend), tiểu luận tiến hành kiểm thử các luồng thao tác thực tế nhằm đảm bảo trải nghiệm nghiệp vụ xuyên suốt. Bảng \ref{tab:blackbox} dưới đây mô tả các kịch bản tiêu biểu:

\begin{table}[H]
\centering
\caption{Danh sách Kịch bản Kiểm thử Hộp đen toàn diện mức Ứng dụng}
\label{tab:blackbox}
\resizebox{\textwidth}{!}{
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/09_Nhap_Link_VNExpress.png}
    \vspace{0.3cm}
    \includegraphics[width=0.8\textwidth]{image/10_Ket_Qua_Phan_Tich_Tin_That_0_Phan_Tram.png}
    \vspace{0.3cm}
    \includegraphics[width=0.8\textwidth]{image/11_Ket_Qua_Tin_That_Cuon_Trang_1.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/07_Nhap_Van_Ban_Tin_Gia_Gom_Chung.png}
    \vspace{0.3cm}
    \includegraphics[width=0.8\textwidth]{image/08_Ket_Qua_Phan_Tich_Tin_Gia_100_Phan_Tram.png}
...
\secA{5.2. Đóng góp của tiểu luận (Tính mới và Ứng dụng)}
Thay vì đi theo lối mòn cố gắng sáng tạo một kiến trúc mạng nơ-ron hoàn toàn mới với rủi ro thiếu ổn định, đóng góp cốt lõi của tiểu luận nằm ở sự \textbf{Đổi mới Kiến trúc Phần mềm (Architectural Innovation)}. Việc tích hợp hệ thống không đơn thuần là ghép nối các module, mà là việc thiết kế thành công một \textbf{Kiến trúc Phân tách Ngữ nghĩa - Kinh nghiệm (Decoupled Semantic-Heuristic Architecture)}. Những giá trị mới mẻ này được đúc kết qua bốn khía cạnh sau:

\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    \item \textbf{Khẳng định tính thích ứng của PhoBERT trong bối cảnh Việt Nam:} Tiểu luận đã chứng minh thực nghiệm rằng kỹ thuật Fine-tuning PhoBERT mang lại F1-Score khả quan (93,42\%) cho bài toán phân loại tin giả tiếng Việt, một mảng ứng dụng chưa được tập trung khai thác trong các hệ thống tin tức điện tử trong nước hiện nay.
    \item \textbf{Đổi mới Kiến trúc Lai ghép (Hybrid Decoupled Architecture):} Khác với các hệ thống thường nhúng cơ chế giải thích thẳng vào cấu trúc mạng nơ-ron (như mạng Attention-based XAI thường khiến tốc độ suy luận chậm đi nhiều lần), tính mới của đề tài là việc \textbf{phân tách độc lập} Động cơ giải thích (Rule-based Heuristics) chạy song song với bộ suy luận học sâu. Khung kiến trúc phân tách này cung cấp lý do cảnh báo minh bạch bằng tiếng Việt mà vẫn giữ vững được \textbf{độ trễ suy luận siêu thấp (Low Latency)}, đáp ứng tốt yêu cầu thời gian thực của ứng dụng Web.
    \item \textbf{Xây dựng hệ thống nguyên mẫu (prototype) (End-to-end System):} Không chỉ dừng lại ở việc huấn luyện mô hình trên môi trường thí nghiệm (Jupyter Notebook), tiểu luận đã phát triển thành công một hệ thống kỹ nghệ phần mềm nguyên mẫu (prototype) bao gồm: Trình thu thập dữ liệu (Web Crawler), Cơ sở dữ liệu SQLite, kiến trúc Backend API bất đồng bộ và giao diện Frontend chuyên nghiệp.
    \item \textbf{Triển khai thành sản phẩm ShieldAI sẵn sàng phục vụ:} Đề tài đã đóng gói toàn bộ quy trình trên thành nền tảng Web có tên gọi ShieldAI, giải quyết trực tiếp nhu cầu phòng chống và kiểm duyệt thông tin sai lệch của cộng đồng người dùng Việt Nam trong kỷ nguyên số.
\end{itemize}

\secA{5.3. Hạn chế của đề tài}
Mặc dù đã đạt được những kết quả khả quan cả về mặt lý thuyết lẫn thực tiễn, tiểu luận vẫn còn tồn tại một số điểm hạn chế mang tính khách quan và chủ quan, cụ thể như sau:


\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    \item \textbf{Nguy cơ Lệch miền dữ liệu (Domain Shift) và Thiên kiến (Dataset Bias):} Mặc dù tập huấn luyện bao gồm hơn 22.000 mẫu, phần lớn dữ liệu được thu thập từ các trang báo chính thống và mạng xã hội ở một thời điểm cụ thể. Điều này tiềm ẩn nguy cơ "thiên kiến nguồn" (Source Bias), khiến mô hình có xu hướng nhận diện tin thật chỉ dựa trên văn phong báo chí tiêu chuẩn thay vì bóc tách ngữ nghĩa thuần túy. Hơn nữa, ngôn ngữ thao túng mạng xã hội biến đổi không ngừng (Domain Shift), các từ lóng hay chiến thuật lừa đảo mới liên tục ra đời có thể làm suy giảm nghiêm trọng độ chính xác của hệ thống theo thời gian nếu không được tái huấn luyện.
    \item \textbf{Hạn chế về phương pháp chia tách nguồn (Source-based Split):} Hệ thống đã sử dụng Stratified Split để kiểm soát rò rỉ dữ liệu (Data Leakage) ở cấp độ từ vựng. Tuy nhiên, việc chia tách ngẫu nhiên chưa đảm bảo tính độc lập hoàn toàn về mặt nguồn phát hay chuỗi sự kiện. Việc các bài viết có cùng chủ đề vô tình xuất hiện ở cả tập Train và Test có thể dẫn đến hiện tượng mô hình "học vẹt" (Memorization) ngữ cảnh sự kiện, khiến hiệu năng kiểm thử có thể bị đánh giá cao hơn năng lực tổng quát hóa thực sự trong môi trường sản xuất.
    \item \textbf{Sự phụ thuộc một phần vào kỹ thuật Web Scraping:} Mặc dù hệ thống đã bổ sung tính năng nhập văn bản trực tiếp để khắc phục, tuy nhiên tính năng tự động thu thập dữ liệu (sử dụng \texttt{BeautifulSoup}) vẫn là một thành phần quan trọng nhằm tối ưu UX. Nếu các tòa soạn báo điện tử tiến hành tái cấu trúc (Redesign) giao diện toàn diện, hoặc kích hoạt các lớp bảo mật chống Bot cực đoan như Cloudflare, module này sẽ tạm thời mất đi khả năng tự động lấy nội dung bài viết thông qua URL.
...
Nhằm khắc phục những hạn chế hiện tại và đưa hệ thống ShieldAI trở thành một nền tảng thương mại hóa hoặc ứng dụng rộng rãi trong cộng đồng, tiểu luận đề xuất các định hướng phát triển cốt lõi trong tương lai như sau:
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    ...
    \item \textbf{Nâng cấp năng lực Thu thập dữ liệu (Data Ingestion):} Phát triển các module thu thập dữ liệu thông minh hơn thông qua việc tích hợp các API chính thức của các nền tảng mạng xã hội lớn (Facebook Graph API, X/Twitter API, Telegram Bot API). Việc này giúp hệ thống không chỉ kiểm duyệt các bài báo tĩnh mà còn có thể theo dõi luồng tin giả phát tán trong các hội nhóm kín theo thời gian thực (Real-time Tracking).
...
\item S. Lee, "Vietnamese Medical Fake News Dataset," Kaggle, 2023. [Online]. Available: \url{https://www.kaggle.com/datasets/leviettrieu369/vietnamese-medical-fake-news-dataset}. [Accessed: Jun. 12, 2026].
...
\item S. Ramírez, "FastAPI framework, high performance, easy to learn, fast to code, ready for production," 2024. [Online]. Available: \url{https://fastapi.tiangolo.com/}. [Accessed: Jun. 12, 2026].
...
\item ISO/IEC, "Systems and software engineering — Systems and software Quality Requirements and Evaluation (SQuaRE) — System and software quality models," \textit{ISO/IEC 25010:2011}, 2011.
\item OWASP Foundation, "REST Security Cheat Sheet," 2024. [Online]. Available: \url{https://cheatsheetseries.owasp.org/}.
...
\textbf{Địa chỉ truy cập mã nguồn (Repository):} \url{https://github.com/Usunase/DoAnTotNghiep.git}

Dưới đây là sơ đồ cấu trúc thư mục cốt lõi của dự án ShieldAI:
\begin{verbatim}
📦 DoAnTotNghiep
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
  ...
  \item \textbf{Phần cứng (Khuyến nghị):} CPU đa nhân (Core i5/Ryzen 5), RAM tối thiểu 8GB (ưu tiên 16GB để chạy mô hình AI mượt mà) và ổ cứng SSD. \textit{Không bắt buộc phải có GPU (Card rời) ở môi trường suy luận (Inference), hệ thống tự động tính toán dự phòng (fallback) về CPU.}
\end{itemize}

\secB{B.2. Hướng dẫn Khởi chạy Toàn bộ Hệ thống}
Hệ thống ShieldAI tuân thủ kiến trúc phân tách (Decoupled Architecture). Tuy nhiên, để tối ưu hóa trải nghiệm người dùng, dự án đã đóng gói toàn bộ quy trình cấu hình và khởi chạy vào các tập lệnh tự động (Bash Scripts).
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
  ...
  \item \textbf{Kiểm chứng Tin tức:} Tại bảng điều khiển trung tâm, người dùng có thể linh hoạt lựa chọn giữa 2 tính năng phân tích (thông qua hệ thống Tab điều hướng):
    \begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
      ...
      \item \textit{Tab Nhập Link (URL):} Sao chép đường dẫn bài báo nghi ngờ và dán vào thanh tìm kiếm. Hệ thống sẽ tự động quét, bóc tách cấu trúc HTML để trích xuất nội dung cốt lõi và loại bỏ quảng cáo dư thừa.
    \end{itemize}
