...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    ...
    \item \textbf{Bước 3: Phân loại (Logits + Softmax):} Mô hình \texttt{AutoModel\allowbreak For\allowbreak Sequence\allowbreak Classification} xuất giá trị Logits, sau đó truyền qua hàm \texttt{Softmax} để tính toán xác suất tin giả; module \texttt{verdict.py} quy đổi 3 mức: $<35\%$ tin thật, 35–74\% đáng ngờ, $\ge 75\%$ tin giả.
...
BEGIN
    ...
    // Bước 3: Phân loại (Hugging Face AutoModel\allowbreak For\allowbreak Sequence\allowbreak Classification)
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    ...
    \item \textbf{Module Suy luận Phân loại (\texttt{phobert\_inference.py}):} Tải PhoBERT-base qua \texttt{Transformers} (PyTorch), trích vector [CLS] 768 chiều, suy luận qua \texttt{AutoModel\allowbreak For\allowbreak Sequence\allowbreak Classification} đã lưu (\texttt{Hugging Face Model}). Logits qua \texttt{Softmax} trả xác suất tin giả; \texttt{verdict.py} quy đổi 3 mức cảnh báo.
