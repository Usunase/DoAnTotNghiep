...
\end{center}
\vfill
\begin{center}{\bfseries\Large LONG AN, THÁNG 06 NĂM 2026}\end{center}
\newpage
\setcounter{page}{2}
...
Kết quả thực nghiệm cho thấy mô hình đạt độ chính xác (Accuracy) \textbf{96.32\%} và F1-Score \textbf{93.42\%}. Các chỉ số này được đánh giá nghiêm ngặt trên tập kiểm thử nội bộ (Test Set) chiếm 12\% (khoảng 2.716 mẫu). Tập dữ liệu được lấy mẫu phân tầng (Stratified Split) từ kho ngữ liệu tổng hợp hơn 22.000 bài báo đã qua bước làm sạch trùng lặp (Deduplication) để giảm thiểu hiện tượng rò rỉ dữ liệu (Data Leakage). Kết quả kiểm thử ngoại lai (External Test) trên các bài báo mới xuất bản năm 2026 cũng đạt 91.5\%, bước đầu chứng minh năng lực khái quát hóa và sự ổn định cao của kiến trúc đề xuất. Hơn thế nữa, hệ thống được triển khai với kiến trúc Client-Server, tích hợp giao diện dễ sử dụng, trực tiếp hỗ trợ người dùng trong việc xây dựng không gian thông tin số an toàn tại Việt Nam.
...
\textbf{g) Phân tích lỗi (Error Analysis)}
...
\begin{itemize}[label={--},leftmargin=0.75cm,itemsep=3pt]
    \item \textbf{Dương tính giả (False Positive - 58 mẫu):} Phân tích chuyên sâu cho thấy có đến $\approx 77.6\%$ (45/58 mẫu) bị nhận diện nhầm là các bài báo chính thống thuộc chuyên mục hình sự, dịch bệnh hoặc thể thao mang tính chất kịch tính (sử dụng tiêu đề "Kinh hoàng", "Sốc"). Nguyên nhân do cấu trúc văn phong lạm dụng ngôn ngữ giật gân (Clickbait) để thu hút người đọc, tạo ra ma trận đặc trưng (Feature Matrix) quá tương đồng với văn bản lừa đảo.
    \item \textbf{Âm tính giả (False Negative - 42 mẫu):} Đáng chú ý, $\approx 66.7\%$ (28/42 mẫu) tin giả lọt lưới được xây dựng với lối hành văn trung lập, trang trọng, mô phỏng hoàn hảo văn phong hành chính của cơ quan nhà nước (ví dụ: thông báo phạt nguội giả mạo). Do hệ thống chủ yếu phân tích văn bản thô (Text-only) và chưa tích hợp Module kiểm chứng sự kiện (Fact-checking) bằng tri thức ngoài (Knowledge Graph), các bài viết tinh vi này vẫn đánh lừa được bộ phân loại.
\end{itemize}
